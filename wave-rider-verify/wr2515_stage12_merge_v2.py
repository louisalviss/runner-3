...
def main():
    ...
    if got!=universe:raise SystemExit(f'meta universe mismatch got={len(got)} expected={len(universe)} missing={sorted(universe-got)[:20]}')
    ...
    setup_times={}
    for s in setup_by:
        setup_by[s].sort();setup_times[s]=[x[0] for x in setup_by[s]]

    union=set();prev_session=None;stage1_snap={};stage2_snap={};selection_rows=[]
    for cp,session_date,mode in CPS:
        now=pass_by.get(cp,set())
        # At the first checkpoint of a new 06:00-VN session, prior session membership is replaced.
        # Between sessions, prior final selection remains effective until this first new checkpoint.
        ...
        ranked=[];lo=cp-LOOK
        for sym in union:
            arr=setup_by.get(sym,[]);tt=setup_times.get(sym,[])
            a=bisect_left(tt,lo);b=bisect_left(tt,cp);vals=[v for _,v in arr[a:b]]
            if len(vals)>=MINSET:ranked.append((p90(vals),sym,len(vals)))
        ranked.sort(key=lambda x:(x[0],x[1]));selected=ranked[:TOPN];stage2_snap[cp]={x[1] for x in selected}
        selection_rows.append({'checkpoint':cp,'session_date':session_date,'mode':mode,'stage1_pass_now':len(now),'stage1_union':len(union),'stage2_eligible':len(ranked),'selected':[{'symbol':s,'p90_required_x':q,'setup_n_14d':n} for q,s,n in selected]})
    ...
    with open(BASE/'trades.jsonl') as f:
        for line in f:
            ...
            st=x['signal_time'];ms=int(datetime.fromisoformat(st.replace('Z','+00:00')).timestamp()*1000) if isinstance(st,str) else int(st)
    ...
    report={
      ...
      'universe_symbols':len(universe),'period':'2025-01-01 to 2026-08-15 exclusive',
      'stage1_reconstruction':'historical candle reconstruction, not archived live ticker snapshots; only completed 5m data before each checkpoint is used',
      'stage1_rule':{'qv24_gte':100000000,'avg_usd_volume_10d_gte':200000000,'vol7_gt':0.06,'adr14_gte':0.05,'checkpoint_modes':['15:30 VN daily','10:00 ET weekdays','12:45 ET weekdays','15:45 ET weekdays'],'session_rollover':'06:00 Asia/Ho_Chi_Minh','semantics':'session union; previous session remains effective until next session first checkpoint'},
      'stage2_rule':{'lookback_calendar_days':14,'min_prior_setups':5,'rank':'lowest P90 Required-X','top_n':5,'uses_pnl':False,'timeframe_minutes':5,'setup_semantics':'actual v2.5.15 signal capable of creating a pending order'},
      ...
      'contamination_note':'W4 was already consumed before this reconstruction. No Stage1/Stage2 threshold is changed using W4. This is research reconstruction, not fresh OOS.',
    ...
    report['stage12_top20_by_net6']=symrows[:20];report['stage12_bottom20_by_net6']=symrows[-20:]
