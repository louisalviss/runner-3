...
def main():
    ...
    for sym,sess in SPECS:
        ...
        row={'symbol':sym,'tf':'10m','n':m['n'],'R':m['R'],'tick':m['tick'],'session':m['session'],'timezone':m['timezone'],'trades':tr}
        rows.append(row); print('NEXT5_10M',row,flush=True)
    out=HERE/'output'/'tv-next5-10m';out.mkdir(parents=True,exist_ok=True)
    (out/'next5-10m.json').write_text(json.dumps(rows,indent=2,default=str))
