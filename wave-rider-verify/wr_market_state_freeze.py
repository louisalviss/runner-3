...
def load_rows(root: Path) -> pd.DataFrame:
    ...
    df = df.sort_values(["signal", "symbol", "tf", "exit"]).reset_index(drop=True)
    return df
...
def main() -> None:
    ...
    if len(df) != 19683:
        raise RuntimeError(f"expected canonical 19683 rows, got {len(df)}")
    if sorted(df["symbol"].unique().tolist()) != sorted(UNIVERSE):
        raise RuntimeError("frozen universe mismatch")

    holdout_ms = int(pd.Timestamp(HOLDOUT_START).timestamp() * 1000)
    if int(df["signal"].max()) >= holdout_ms:
        raise RuntimeError("training rows overlap holdout")
    ...
    threshold = float(np.quantile(pred, 0.70))

    bio = io.BytesIO()
    joblib.dump(model, bio, compress=3)
    ...
    manifest = {
        ...
        "source_market_state_run_id": SOURCE_RUN_ID,
        "training_rows": int(len(df)),
        "training_symbols": int(df["symbol"].nunique()),
        "training_data_end_exclusive_utc": TRAINING_DATA_END_EXCLUSIVE,
        "holdout_start_utc": HOLDOUT_START,
        "holdout_end_utc": HOLDOUT_END,
        "features": FEATURES,
        "universe": UNIVERSE,
        "timeframes_minutes": [5, 10],
        "target": "net_1bps",
        "selector": "prediction > frozen training-prediction q70",
        "selector_threshold": threshold,
        "train_selected_n": int((pred > threshold).sum()),
        "model": {
            "type": "Pipeline(SimpleImputer(median), HistGradientBoostingRegressor)",
            "params": {
                "max_iter": 100,
                "learning_rate": 0.05,
                "max_depth": 2,
        ...
        "primary_gate_review": {
            "not_before_utc": "2027-01-01T00:00:00Z",
            "minimum_selected_trades": 100,
            "net_1bps_R_gt": 0.0,
            "PF_1bps_gte": 1.20,
            "max_DD_R_gte": -20.0,
            "minimum_distinct_symbols": 15,
            "max_single_symbol_abs_R_share": 0.25,
            "annualized_return_is_not_a_primary_gate": True,
        },
