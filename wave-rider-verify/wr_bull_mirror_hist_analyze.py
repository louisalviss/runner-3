...
# Official CPI / Employment Situation release dates and FOMC decision dates in ET.
EVENTS={
 '2022-01-07','2022-01-12','2022-01-26','2022-02-04','2022-02-10','2022-03-04','2022-03-10','2022-03-16',
 '2022-04-01','2022-04-12','2022-05-04','2022-05-06','2022-05-11','2022-06-03','2022-06-10','2022-06-15',
 '2022-07-08','2022-07-13','2022-07-27','2022-08-05','2022-08-10','2022-09-02','2022-09-13','2022-09-21',
 '2022-10-07','2022-10-13','2022-11-02','2022-11-04','2022-11-10','2022-12-02','2022-12-13','2022-12-14',
 '2023-01-06','2023-01-12','2023-02-01','2023-02-03','2023-02-14','2023-03-10','2023-03-14','2023-03-22',
 '2023-04-07','2023-04-12','2023-05-03','2023-05-05','2023-05-10','2023-06-02','2023-06-13','2023-06-14',
 '2023-07-07','2023-07-12','2023-07-26','2023-08-04','2023-08-10','2023-09-01','2023-09-13','2023-09-20',
 '2023-10-06','2023-10-12','2023-11-01','2023-11-03','2023-11-14','2023-12-08','2023-12-12','2023-12-13'
}
...
def t_on(a):
    ...
    return any((d-e).days in TON_OFFSETS for e in EVENT_DATES if -3 <= (d-e).days <= 3)
...
if not T: raise SystemExit('no 2022-23 Zone C T-ON trades')

def getzip(url,tries=3):
    for i in range(tries):
        try:
            r=requests.get(url,timeout=25,headers={'User-Agent':'runner3-wr-bull-historical/1.0'})
...
first,last=dt(T[0]),dt(T[-1]); sy,sm=first.year,first.month
...
def market_feats(ts):
    ...
    return {'btc':btc,'vol':vol,'breadth':breadth,'breadth_n':len(ups)}
...
def prev_months_for_funding(a):
    d=dt(a); return {(d.year,d.month),month_prev(d.year,d.month)}
need=defaultdict(set)
for a in X: need[a['symbol']]|=prev_months_for_funding(a)
...
def desc(xs):
    return {
      ...
      'years':{str(y):{str(b):met([a for a in xs if dt(a).year==y],b) for b in (6,8,10,12)} for y in (2022,2023)},
...
report={
 'status':'WR_BULL_MIRROR_2022_2023_COMPLETE',
 'scope':{'tf':'10m','zone_c_vn':'23:00-00:59','t_on':[-2,-1,0,2,3],'raw_zonec_trades':sum(1 for p in glob.glob('/tmp/all/trades-*.jsonl') for _ in open(p)),'t_on_trades':len(T),'market_covered':len(X),'funding3_covered':len(F),'first':dt(T[0]).isoformat(),'last':dt(T[-1]).isoformat()},
 ...
 'definitions':{'btc':'prior-closed daily 30d return >0 = UP','vol':'BTC prior-closed daily RV20 > RV60 = HIGH; <= = LOW','breadth':'fixed 20-asset prior-closed 20d positive-return share >50%=UP else DOWN, minimum 10 valid','funding3':'mean last 3 funding observations strictly before trade; >0=POS else NONPOS','primary_mirror':'exact complement mirror of frozen C3 market gate; WR entry direction unchanged'},
 'warning':'Historical retrospective diagnostic. Primary symmetric mirror is predeclared; the other seven bull cells remain diagnostics only. Same frozen current-universe methodology implies survivorship/listing-history limitations.'
}
