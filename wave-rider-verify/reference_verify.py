...
def next_bracket(plan,x,start_at=None):
    # Return (reason,price) using TV historical OHLC path heuristic. If start_at is
    # None the position existed before bar open. If set, ignore path before entry.
...
def run(tf,bars,tick,start_ms,end_ms):
    ...
    for i,x in enumerate(bars):
        ...
        closed=False
        # Active position from previous bar: bracket first.
        if active is not None:
            r,px=next_bracket(active,x,None)
            if r:
                diag['tp' if r=='TP' else 'sl']+=1; closed=close_trade(i,r,px)
        # Pending stop entry is valid only on exactly the next chart candle.
        ...
        # Warmup bars compute indicators only; no orders until evaluation window.
...
def main():
    ...
    master=dict(strategy='Wave Rider 2.5.13 independent reference',source='Binance USD-M 1m public daily klines',symbol=SYMBOL,start=START,end=END,tick=tick,missing=missing,summaries=sm,
        caveats=['Same 1m source resampled to all timeframes','UTC Binance session-close model','TradingView OHLC path heuristic; Canon both-touch bar forced to -1R','Pivot ties suppressed and counted'])
