...
def tv_fetch_more(symbol,session='regular',total=base.TARGET_BARS):
    cs=base.sid('cs_')
    ...
    send('resolve_symbol',[cs,'sym_1',cfg]);send('create_series',[cs,'s1','s1','sym_1',base.TF,min(5000,total),''])
    rows={};info=None;completed=0;requests=0;deadline=time.time()+180
    def take(container):
        if not isinstance(container,dict):return
        candidates=[]
        for k in ('s1','sds_1'):
            if isinstance(container.get(k),dict):candidates.append(container[k])
        candidates += [v for v in container.values() if isinstance(v,dict) and isinstance(v.get('s'),list)]
        seen=set()
        for ser in candidates:
            if id(ser) in seen:continue
            seen.add(id(ser))
    ...
    while time.time()<deadline:
        ...
        for kind,msg in base.unpack(raw):
            ...
            if m in ('series_error','symbol_error','critical_error'):raise RuntimeError(f'{symbol} {session} {m}: {p}')
            if m=='series_completed':
                completed+=1
                print('BATCH',symbol,session,'completed',completed,'rows',len(rows),flush=True)
                if len(rows)<total and requests<8:
                    n=min(5000,total-len(rows));send('request_more_data',[cs,'s1',n]);requests+=1
                else:
                    deadline=0;break
    try:ws.close()
    except Exception:pass
    if not rows:raise RuntimeError(f'{symbol} {session}: no bars')
    bars=[base.Bar(ts*1000,ts*1000+base.TF_MS,*rows[ts]) for ts in sorted(rows)]
    print('HISTORY',symbol,session,'rows',len(bars),'first',bars[0].ot,'last',bars[-1].ot,'requests',requests,flush=True)
    return bars,info or {}

base.tv_fetch=tv_fetch_more
base.main()
