...
s=p.read_text()
repl={
 "STATE=int(datetime(2024,12,1,tzinfo=timezone.utc).timestamp()*1000)":"STATE=int(datetime(2021,12,1,tzinfo=timezone.utc).timestamp()*1000)",
 "START=int(datetime(2025,1,1,tzinfo=timezone.utc).timestamp()*1000)":"START=int(datetime(2022,1,1,tzinfo=timezone.utc).timestamp()*1000)",
 ...
 "months=[(2024,12)]+[(2025,m) for m in range(1,13)]+[(2026,m) for m in range(1,8)]":"months=[(2021,12)]+[(2022,m) for m in range(1,13)]+[(2023,m) for m in range(1,13)]",
}
