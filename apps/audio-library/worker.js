...
async function bulkShare(){const arr=selectedItems().filter(i=>i.status==='ready');if(!arr.length)return toast('Chưa chọn audio sẵn sàng');const text=arr.map(i=>(i.title||'Audio')+'\n'+(i.audioUrl||i.sourceUrl)).join('\n\n');try{if(navigator.share)await navigator.share({title:arr.length+' audio',text});else{await navigator.clipboard.writeText(text);toast('Đã sao chép '+arr.length+' link')}}catch(e){if(e?.name!=='AbortError')console.error(e)}}
