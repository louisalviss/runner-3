...
async function userAuthorized(request, env) {
  const auth = request.headers.get('authorization') || '';
  const token = auth.startsWith('Bearer ') ? auth.slice(7) : '';
  if (!token || !env.LIBRARY_ACCESS_SHA256) return false;
  const suppliedHash = await sha256Hex(token);
  return timingSafeEqualStrings(suppliedHash, env.LIBRARY_ACCESS_SHA256);
}

async function runnerAuthorized(request, env) {
  const token = request.headers.get('x-runner-token') || '';
  if (!token || !env.RUNNER_SHARED_TOKEN) return false;
  return timingSafeEqualStrings(token, env.RUNNER_SHARED_TOKEN);
}
...
async function deleteItem(env, id) {
  ...
  const mediaPrefix = item?.mediaPrefix || `${MEDIA_PREFIX}${id}/`;
  await deletePrefix(env.AUDIO_BUCKET, mediaPrefix);
...
function validHttpUrl(value) {
  try {
    ...
    return (u.protocol === 'https:' || u.protocol === 'http:') && u.hostname.length > 1;
  } catch {
    return false;
  }
}
...
async function addItem(request, env) {
  ...
  if (duplicate) return json({ item: duplicate, duplicate: true }, 200);
  ...
  const item = {
    ...
    createdAt: now,
    updatedAt: now,
    expiresAt: addDaysIso(Number(env.DEFAULT_RETENTION_DAYS || 30)),
    pinned: false,
    durationSeconds: null,
    progressSeconds: 0,
    audioUrl: null,
    transcriptUrl: null,
    mediaPrefix: `${MEDIA_PREFIX}${id}/`,
    error: null,
  };
...
async function updateProgress(request, env, id) {
  const item = await getJsonObject(env.AUDIO_BUCKET, itemKey(id));
  if (!item) return json({ error: 'Không tìm thấy audio' }, 404);
  let body;
  try { body = await request.json(); } catch { return json({ error: 'JSON không hợp lệ' }, 400); }
  const seconds = Math.max(0, Number(body?.seconds || 0));
  const duration = Math.max(0, Number(body?.duration || item.durationSeconds || 0));
  item.progressSeconds = Number.isFinite(seconds) ? seconds : 0;
  if (duration > 0) item.durationSeconds = duration;
  item.lastPlayedAt = new Date().toISOString();
  item.updatedAt = item.lastPlayedAt;
  if (item.durationSeconds && item.progressSeconds >= item.durationSeconds - 3) {
  ...
  await putJson(env.AUDIO_BUCKET, itemKey(id), item);
  return json({ item: publicItem(item) });
}
...
async function updateItem(request, env, id) {
  ...
  if (typeof body?.pinned === 'boolean') {
    ...
    item.expiresAt = body.pinned ? null : addDaysIso(Number(env.DEFAULT_RETENTION_DAYS || 30));
  }
...
async function runnerNext(env) {
  const queueKeys = await listKeys(env.AUDIO_BUCKET, QUEUE_PREFIX, 100);
  if (!queueKeys.length) return new Response(null, { status: 204 });
  queueKeys.sort();
  for (const key of queueKeys) {
    const queued = await getJsonObject(env.AUDIO_BUCKET, key);
    ...
    item.updatedAt = new Date().toISOString();
    item.error = null;
    await putJson(env.AUDIO_BUCKET, itemKey(item.id), item);
    return json({ item: publicItem(item) });
  }
...
async function runnerComplete(request, env) {
  ...
  const item = await getJsonObject(env.AUDIO_BUCKET, itemKey(id));
  if (!item) return json({ error: 'Không tìm thấy item' }, 404);
  item.status = 'ready';
  item.title = String(body.title || item.title || item.sourceLabel).slice(0, 240);
  item.sourceLabel = String(body.sourceLabel || item.sourceLabel || 'Web').slice(0, 80);
  item.durationSeconds = Math.max(0, Number(body.durationSeconds || 0)) || null;
  item.audioUrl = String(body.audioUrl || '');
  item.transcriptUrl = body.transcriptUrl ? String(body.transcriptUrl) : null;
  item.truncated = Boolean(body.truncated);
  item.updatedAt = new Date().toISOString();
  item.error = null;
  await putJson(env.AUDIO_BUCKET, itemKey(id), item);
  await env.AUDIO_BUCKET.delete(queueKey(id));
  return json({ item: publicItem(item) });
}
...
async function runnerFail(request, env) {
  ...
  if (!item) return json({ error: 'Không tìm thấy item' }, 404);
  item.status = 'error';
  item.error = String(body.error || 'Không thể xử lý link').slice(0, 180);
  item.errorDetail = String(body.detail || '').slice(0, 1200);
  item.updatedAt = new Date().toISOString();
  await putJson(env.AUDIO_BUCKET, itemKey(id), item);
  await env.AUDIO_BUCKET.delete(queueKey(id));
  return json({ item: publicItem(item) });
...
async function purgeExpired(env) {
  ...
  const now = Date.now();
  const expired = items.filter((item) => !item.pinned && item.expiresAt && Date.parse(item.expiresAt) <= now);
  for (const item of expired) await deleteItem(env, item.id);
  console.log(JSON.stringify({ event: 'audio_library_purge', deleted: expired.length }));
}

const APP_HTML = `<!doctype html>
<html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#0b0d11"><title>Audio Library</title>
<style>
:root{color-scheme:dark;--bg:#0b0d11;--card:#151820;--line:#242833;--text:#f5f6f8;--muted:#939aa7;--accent:#8b5cf6;--danger:#ef6666}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:15px/1.45 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}button,input{font:inherit}button{cursor:pointer}main{max-width:640px;margin:auto;padding:calc(18px + env(safe-area-inset-top)) 16px calc(135px + env(safe-area-inset-bottom))}.top{display:flex;align-items:center;justify-content:space-between;margin:4px 0 18px}.top h1{font-size:26px;margin:0}.add{width:100%;border:0;border-radius:13px;background:var(--accent);color:white;padding:14px 16px;font-weight:650;margin-bottom:16px}.tabs{display:flex;gap:6px;margin-bottom:12px}.tab{border:1px solid var(--line);background:transparent;color:var(--muted);padding:8px 11px;border-radius:10px}.tab.on{background:#201a31;color:white;border-color:#362954}.list{display:grid;gap:9px}.item{position:relative;background:var(--card);border:1px solid var(--line);border-radius:14px;padding:13px 42px 12px 13px}.item h3{font-size:16px;line-height:1.3;margin:0 0 5px;max-width:92%}.meta{font-size:13px;color:var(--muted)}.state{font-size:13px;margin-top:7px;color:#b6bdc9}.state.error{color:var(--danger)}.bar{height:4px;background:#272b35;border-radius:99px;overflow:hidden;margin-top:11px}.fill{height:100%;background:var(--accent);width:0}.more{position:absolute;right:8px;top:8px;border:0;background:transparent;color:#aeb5c1;font-size:22px;padding:6px}.empty{text-align:center;color:var(--muted);padding:42px 12px}.player{position:fixed;left:50%;transform:translateX(-50%);bottom:0;width:min(640px,100%);background:#11141b;border-top:1px solid var(--line);padding:10px 16px calc(10px + env(safe-area-inset-bottom));display:none}.player.show{display:block}.ptitle{font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:7px}.timeline{width:100%}.controls{display:flex;justify-content:center;gap:28px;align-items:center;margin-top:4px}.controls button{border:0;background:transparent;color:white;font-size:17px}.controls .play{width:46px;height:46px;border-radius:50%;background:var(--accent);font-size:19px}.sheet,.login{position:fixed;inset:0;background:#0009;display:none;align-items:flex-end;justify-content:center;z-index:5}.sheet.show,.login.show{display:flex}.panel{width:min(640px,100%);background:#171a21;border-radius:18px 18px 0 0;padding:18px 16px calc(18px + env(safe-area-inset-bottom))}.panel h3{margin:0 0 14px}.panel button,.panel input{width:100%;border-radius:12px;padding:13px;margin-top:8px}.panel button{border:1px solid var(--line);background:#20242d;color:white}.panel .danger{color:#ff7777}.login{align-items:center;padding:16px}.login .panel{border-radius:16px;max-width:420px}.panel input{border:1px solid var(--line);background:#0e1015;color:white}.small{font-size:12px;color:var(--muted);margin-top:6px}
</style></head><body><main><div class="top"><h1>Thư viện</h1><span id="count" class="meta"></span></div><button class="add" id="add">＋ Thêm link</button><div class="tabs"><button class="tab on" data-filter="all">Tất cả</button><button class="tab" data-filter="listening">Đang nghe</button><button class="tab" data-filter="done">Đã nghe</button></div><div id="list" class="list"></div></main>
...
<div id="login" class="login"><div class="panel"><h3>Mã truy cập</h3><input id="key" type="password" autocomplete="off" placeholder="Dán mã truy cập"><button id="loginBtn">Mở thư viện</button><div class="small">Mã chỉ lưu trên trình duyệt này.</div></div></div>
<script>
const $=s=>document.querySelector(s), $$=s=>[...document.querySelectorAll(s)];
const audio=$('#audio'), player=$('#player'), timeline=$('#timeline'); let items=[], playing=null, sheetItem=null, filter='all', saveTimer=0;
const qp=new URLSearchParams(location.search); if(qp.get('key')){localStorage.setItem('audio-library-key',qp.get('key'));history.replaceState({},'',location.pathname)}
let token=localStorage.getItem('audio-library-key')||'';
...
function daysLeft(x){if(!x)return '';return Math.max(0,Math.ceil((Date.parse(x)-Date.now())/86400000))}
async function api(path,opt={}){const r=await fetch(path,{...opt,headers:{...auth(),...(opt.headers||{})}});if(r.status===401){$('#login').classList.add('show');throw new Error('auth')}if(r.status===204)return null;const d=await r.json();if(!r.ok)throw new Error(d.error||'Lỗi');return d}
async function load(){try{const d=await api('/api/items');items=d.items||[];render()}catch(e){if(e.message!=='auth')alert(e.message)}}
function visible(i){if(filter==='done')return i.listened;if(filter==='listening')return i.status==='ready'&&!i.listened&&(i.progressSeconds||0)>0;return true}
function render(){const list=$('#list');const rows=items.filter(visible);$('#count').textContent=items.length?items.length+' audio':'';if(!rows.length){list.innerHTML='<div class="empty">Chưa có audio.</div>';return}list.innerHTML=rows.map(i=>{const p=i.durationSeconds?Math.min(100,(i.progressSeconds||0)/i.durationSeconds*100):0;let state=i.status==='pending'?'Đang chờ xử lý…':i.status==='processing'?'Đang tạo MP3…':i.status==='error'?'Lỗi: '+(i.error||'Không xử lý được'):i.listened?'Đã nghe xong':(i.progressSeconds||0)>0?'Đã nghe '+fmt(i.progressSeconds)+' / '+fmt(i.durationSeconds):'Chưa nghe';const exp=i.pinned?' · Giữ lại':i.expiresAt?' · Xóa sau '+daysLeft(i.expiresAt)+' ngày':'';return '<div class="item" data-id="'+i.id+'"><h3>'+esc(i.title||i.sourceLabel)+'</h3><div class="meta">'+esc(i.sourceLabel||'Web')+(i.durationSeconds?' · '+fmt(i.durationSeconds):'')+exp+'</div><div class="state '+(i.status==='error'?'error':'')+'">'+esc(state)+'</div>'+(i.status==='ready'?'<div class="bar"><div class="fill" style="width:'+p.toFixed(1)+'%"></div></div>':'')+'<button class="more" data-more="'+i.id+'">⋯</button></div>'}).join('');$$('.item').forEach(el=>el.onclick=e=>{if(e.target.dataset.more)return;const i=items.find(x=>x.id===el.dataset.id);if(i?.status==='ready')playItem(i)});$$('[data-more]').forEach(b=>b.onclick=e=>{e.stopPropagation();openSheet(items.find(x=>x.id===b.dataset.more))})}
function esc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
async function addLink(){const url=prompt('Dán link cần chuyển thành MP3');if(!url)return;try{await api('/api/items',{method:'POST',body:JSON.stringify({url})});await load()}catch(e){if(e.message!=='auth')alert(e.message)}}
...
async function handleFetch(request, env) {
  ...
  if (url.pathname.startsWith('/api/')) {
    if (!(await userAuthorized(request, env))) return json({ error: 'Unauthorized' }, 401);
...
export default {
  async fetch(request, env) {
    try {
      ...
      console.error(JSON.stringify({ event: 'audio_library_error', message: String(error?.stack || error) }));
