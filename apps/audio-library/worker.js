...
:root{color-scheme:dark;--bg:#0b0d11;--card:#151820;--line:#242833;--text:#f5f6f8;--muted:#939aa7;--accent:#8b5cf6;--danger:#ef6666;--surface:#11141b}*{box-sizing:border-box}html,body{width:100%;height:100%;margin:0;overflow:hidden;background:var(--bg)}body{color:var(--text);font:15px/1.45 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;-webkit-text-size-adjust:100%;overscroll-behavior:none}button,input,select{font:inherit}button{cursor:pointer}.app{width:100%;max-width:680px;height:100dvh;min-height:100%;margin:0 auto;display:flex;flex-direction:column;overflow:hidden;background:var(--bg)}.head{flex:0 0 auto;padding:calc(14px + env(safe-area-inset-top)) 14px 0;background:var(--bg);z-index:2}.top{display:flex;align-items:center;justify-content:space-between;gap:12px;margin:0 2px 12px}.top h1{font-size:24px;line-height:1.15;margin:0;letter-spacing:-.02em}.meta{font-size:12px;color:var(--muted);white-space:nowrap}.addform{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;margin-bottom:12px}.urlinput{width:100%;min-width:0;height:46px;border:1px solid var(--line);border-radius:12px;background:#11141a;color:var(--text);padding:0 13px;outline:none}.urlinput:focus{border-color:#5f46a4;box-shadow:0 0 0 3px #8b5cf61c}.submit{height:46px;min-width:84px;border:0;border-radius:12px;background:var(--accent);color:#fff;font-weight:700;padding:0 15px}.submit:disabled{opacity:.55;cursor:default}.tabs{display:flex;gap:6px;padding-bottom:10px;overflow-x:auto;scrollbar-width:none}.tabs::-webkit-scrollbar{display:none}.tab{flex:0 0 auto;border:1px solid var(--line);background:transparent;color:var(--muted);padding:8px 11px;border-radius:10px}.tab.on{background:#201a31;color:#fff;border-color:#362954}.viewport{flex:1 1 auto;min-height:0;overflow-y:auto;overflow-x:hidden;overscroll-behavior:contain;-webkit-overflow-scrolling:touch;padding:2px 14px 14px}.list{display:grid;gap:9px;min-width:0}.item{position:relative;min-width:0;background:var(--card);border:1px solid var(--line);border-radius:14px;padding:13px 42px 12px 13px}.item h3{font-size:16px;line-height:1.3;margin:0 0 5px;max-width:100%;overflow-wrap:anywhere}.state{font-size:13px;margin-top:7px;color:#b6bdc9}.state.error{color:var(--danger)}.bar{height:4px;background:#272b35;border-radius:99px;overflow:hidden;margin-top:11px}.fill{height:100%;background:var(--accent)}.more{position:absolute;right:7px;top:7px;border:0;background:transparent;color:#aeb5c1;font-size:22px;padding:6px}.empty{text-align:center;color:var(--muted);padding:34px 12px}.player{flex:0 0 auto;display:none;background:var(--surface);border-top:1px solid var(--line);box-shadow:0 -10px 30px #0005;padding:10px 14px calc(10px + env(safe-area-inset-bottom));z-index:3}.player.show{display:block}.playerhead{display:flex;align-items:center;gap:10px;margin-bottom:7px}.ptitle{flex:1;min-width:0;font-weight:650;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.speed{height:32px;border:1px solid var(--line);border-radius:9px;background:#1b1f28;color:#fff;padding:0 8px}.timeline{display:block;width:100%;margin:0;accent-color:var(--accent)}.timebar{display:flex;justify-content:space-between;color:var(--muted);font-size:11px;margin-top:1px}.controls{display:flex;justify-content:center;gap:26px;align-items:center;margin-top:2px}.controls button{border:0;background:transparent;color:#fff;font-size:16px;min-width:42px;height:42px}.controls .play{width:46px;height:46px;border-radius:50%;background:var(--accent);font-size:18px;min-width:46px}.sheet{position:fixed;inset:0;background:#0009;display:none;align-items:flex-end;justify-content:center;z-index:10}.sheet.show{display:flex}.panel{width:min(680px,100%);background:#171a21;border-radius:18px 18px 0 0;padding:18px 16px calc(18px + env(safe-area-inset-bottom))}.panel h3{margin:0 0 14px}.panel button{width:100%;border-radius:12px;padding:13px;margin-top:8px;border:1px solid var(--line);background:#20242d;color:#fff}.panel .danger{color:#ff7777}@media(max-width:430px){.head{padding-left:12px;padding-right:12px}.viewport{padding-left:12px;padding-right:12px}.player{padding-left:12px;padding-right:12px}.top h1{font-size:22px}.submit{min-width:76px;padding:0 12px}.controls{gap:20px}}
</style></head><body><div class="app">
<header class="head"><div class="top"><h1>Thư viện</h1><span id="count" class="meta"></span></div><form id="addForm" class="addform"><input id="urlInput" class="urlinput" type="url" inputmode="url" autocomplete="off" autocapitalize="off" spellcheck="false" placeholder="Dán link Reddit, YouTube, X, bài viết…" required><button id="submit" class="submit" type="submit">Thêm</button></form><div class="tabs"><button class="tab on" data-filter="all" type="button">Tất cả</button><button class="tab" data-filter="listening" type="button">Đang nghe</button><button class="tab" data-filter="done" type="button">Đã nghe</button></div></header>
<main class="viewport"><div id="list" class="list"></div></main>
<div id="player" class="player"><div class="playerhead"><div id="ptitle" class="ptitle"></div><select id="speed" class="speed" aria-label="Tốc độ phát"><option value="0.75">0.75×</option><option value="1">1×</option><option value="1.25">1.25×</option><option value="1.5">1.5×</option><option value="1.75">1.75×</option><option value="2">2×</option></select></div><input id="timeline" class="timeline" type="range" min="0" max="100" value="0" step="0.1"><div class="timebar"><span id="elapsed">0:00</span><span id="duration">0:00</span></div><div class="controls"><button id="back" type="button">−15</button><button id="play" class="play" type="button">▶</button><button id="forward" type="button">+15</button></div><audio id="audio" playsinline preload="metadata"></audio></div>
</div><div id="sheet" class="sheet"><div class="panel"><h3 id="sheetTitle"></h3><button id="pin"></button><button id="delete" class="danger">Xóa audio</button><button id="close">Đóng</button></div></div>
<script>
const $=s=>document.querySelector(s),$$=s=>[...document.querySelectorAll(s)];const audio=$('#audio'),player=$('#player'),timeline=$('#timeline'),speed=$('#speed'),urlInput=$('#urlInput'),submit=$('#submit');let items=[],playing=null,sheetItem=null,filter='all',saveTimer=0;
...
async function addLink(url){url=String(url||'').trim();if(!url)return;submit.disabled=true;const old=submit.textContent;submit.textContent='Đang thêm…';try{const d=await api('/api/items',{method:'POST',body:JSON.stringify({url})});urlInput.value='';await load();if(d?.duplicate)urlInput.placeholder='Link này đã có trong thư viện'}catch(e){alert(e.message)}finally{submit.disabled=false;submit.textContent=old;urlInput.focus()}}
