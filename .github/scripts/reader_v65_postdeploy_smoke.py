...
def get(path):
    req=urllib.request.Request(CORE+path,headers={'User-Agent':'runner3-reader-v65-smoke/1.3','Cache-Control':'no-cache'})
...
for marker in ['data-r3-library-v56="1"','r3InstallMainManageV65','r3HydrateServerProgressV65','window.__R3_EDIT_NAME_UX_V653=true','r3-edit-layer-v653','Đổi tên sách']:
...
for marker in [
    ...
    'r3MergeRemoteProgressV65','r3InstallLiveManageV65','window.__R3_EDIT_NAME_UX_V653=true','r3-edit-layer-v653','Đổi tên sách',
...
for path in ['/artifact-library/api/progress','/artifact-library/api/manage']:
    req=urllib.request.Request(CORE+path,method='GET' if path.endswith('progress') else 'POST',headers={'User-Agent':'runner3-reader-v65-smoke/1.3'})
    try:
...
print('READER_V65_3_SERVER_SMOKE=PASS')
