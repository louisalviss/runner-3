...
function runner3_edge_options() {
    return wp_parse_args((array) get_option(RUNNER3_EDGE_OPTION, array()), runner3_edge_defaults());
}

function runner3_edge_sanitize($input) {
    $current = runner3_edge_options();
    $input = (array) $input;
...
function runner3_edge_register_settings() {
...
function runner3_edge_safe_status() {
    return wp_parse_args((array) get_option(RUNNER3_EDGE_STATUS_OPTION, array()), array(
        'ok' => null,
        'http' => null,
        'at' => '',
        'reason' => '',
...
function runner3_edge_settings_page() {
    ...
    <div class="wrap">
        ...
        <hr>
        <h2>Manual purge</h2>
        <p><a class="button button-secondary" href="<?php echo esc_url(wp_nonce_url(admin_url('admin-post.php?action=runner3_edge_manual_purge'), 'runner3_edge_manual_purge')); ?>">Purge public HTML cache now</a></p>
        <p><strong>Last result:</strong> <?php
            if ($status['ok'] === null) echo 'Not run yet';
            else echo $status['ok'] ? 'OK' : 'FAILED';
            if ($status['http'] !== null) echo ' · HTTP ' . esc_html((string) $status['http']);
            if ($status['at']) echo ' · ' . esc_html($status['at']);
            if ($status['reason']) echo ' · ' . esc_html($status['reason']);
            if ($status['detail']) echo ' · ' . esc_html($status['detail']);
        ?></p>
        <p><strong>Rollback:</strong> uncheck the master switch. This stops future purge calls without changing content, theme files or the Cloudflare Worker.</p>
    </div>
    <?php
}

function runner3_edge_record_status($ok, $http, $reason, $detail = '') {
    update_option(RUNNER3_EDGE_STATUS_OPTION, array(
        'ok' => (bool) $ok,
        'http' => $http === null ? null : (int) $http,
        'at' => gmdate('c'),
        'reason' => sanitize_text_field((string) $reason),
        'detail' => sanitize_text_field(substr((string) $detail, 0, 240)),
    ), false);
}

function runner3_edge_url_path($url) {
...
function runner3_edge_purge($reason = 'wordpress-change', $urls = array(), $force = false) {
    ...
    foreach ((array) $urls as $url) {
        $path = runner3_edge_url_path($url);
    ...
    if (is_wp_error($response)) {
        runner3_edge_record_status(false, null, $reason, $response->get_error_code());
        return false;
    }
...
function runner3_edge_manual_purge() {
    if (!current_user_can('manage_options')) wp_die('Forbidden');
    check_admin_referer('runner3_edge_manual_purge');
    runner3_edge_purge('manual-admin', array(home_url('/')), true);
    wp_safe_redirect(add_query_arg(array('page' => RUNNER3_EDGE_PAGE, 'purged' => 1), admin_url('options-general.php')));
    exit;
}
add_action('admin_post_runner3_edge_manual_purge', 'runner3_edge_manual_purge');

function runner3_edge_post_change($post_id, $post = null) {
    if (wp_is_post_revision($post_id) || wp_is_post_autosave($post_id)) return;
    $post = $post ?: get_post($post_id);
    if (!$post || $post->post_status !== 'publish') return;
    runner3_edge_purge('post-save', array(get_permalink($post_id), home_url('/')));
}
add_action('save_post', 'runner3_edge_post_change', 20, 2);

function runner3_edge_transition($new_status, $old_status, $post) {
    if (!$post || ($new_status !== 'publish' && $old_status !== 'publish')) return;
    runner3_edge_purge('post-status-change', array(get_permalink($post), home_url('/')));
}
add_action('transition_post_status', 'runner3_edge_transition', 20, 3);

function runner3_edge_post_removed($post_id) {
    runner3_edge_purge('post-remove', array(home_url('/')));
}
add_action('trashed_post', 'runner3_edge_post_removed');
add_action('deleted_post', 'runner3_edge_post_removed');
add_action('untrashed_post', 'runner3_edge_post_removed');

function runner3_edge_global_change() {
    runner3_edge_purge('site-structure-change', array(home_url('/')));
}
add_action('wp_update_nav_menu', 'runner3_edge_global_change');
add_action('switch_theme', 'runner3_edge_global_change');
add_action('customize_save_after', 'runner3_edge_global_change');

function runner3_edge_comment_change() {
    runner3_edge_purge('comment-change', array(home_url('/')));
}
add_action('comment_post', 'runner3_edge_comment_change');
add_action('edit_comment', 'runner3_edge_comment_change');
add_action('wp_set_comment_status', 'runner3_edge_comment_change');
