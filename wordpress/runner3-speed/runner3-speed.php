 ...
 * Description: One-click, fail-safe full-page acceleration for WordPress. ON caches safe anonymous HTML; OFF restores the normal WordPress path.
 * Version: 1.1.1
 * Author: Runner3
 */
...
final class Runner3_Speed {
    ...
    public static function page() {
        ...
        $detail = is_array($s) && !empty($s['detail']) ? $s['detail'] : ($on ? 'Safe cache active.' : 'WordPress is serving normally.');
        ...
        echo '<p>✓ Guest page cache<br>✓ Login/Admin/API bypass<br>✓ Cart/Checkout/session bypass<br>✓ Auto purge after content changes<br>✓ Version-locked cache files<br>✓ OFF = normal WordPress path</p>';
        echo '</div></div>';
    }
    ...
    public static function toggle() {
        ...
        $reason = self::enable();
        self::status($reason === '' ? 'on' : 'blocked', $reason === '' ? 'Performance ON. Safe anonymous page cache active.' : $reason);
        self::redirect();
    }
