...
$marker = 'RUNNER3_SPEED_DROPIN';
$dropin = WP_CONTENT_DIR . '/advanced-cache.php';
if (is_file($dropin)) {
    $head = @file_get_contents($dropin, false, null, 0, 4096);
    if (is_string($head) && strpos($head, $marker) !== false) @unlink($dropin);
}

$dir = WP_CONTENT_DIR . '/cache/runner3-speed';
if (is_dir($dir)) {
...
delete_option('runner3_edge_optimizer_pending');
...
foreach ($paths as $path) {
    ...
    $updated = preg_replace("/\/\* RUNNER3_SPEED_WP_CACHE \*\/\s*define\(\s*['\"]WP_CACHE['\"]\s*,\s*true\s*\);\s*/", '', $content, 1, $count);
    if ($count === 1 && is_string($updated)) {
        $tmp = dirname($path) . '/.runner3-uninstall-' . uniqid('', true) . '.tmp';
        if (file_put_contents($tmp, $updated, LOCK_EX) !== false) @rename($tmp, $path);
