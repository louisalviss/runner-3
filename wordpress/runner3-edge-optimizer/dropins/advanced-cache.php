...
$runner3_mtime=@filemtime($runner3_file);$runner3_size=@filesize($runner3_file);if(!$runner3_mtime||(time()-$runner3_mtime)>3600||$runner3_size===false||$runner3_size<512){@unlink($runner3_file);if(!headers_sent())header('X-Runner3-Speed: MISS');return;}
if(!headers_sent()){header('Content-Type: text/html; charset=UTF-8');header('Cache-Control: public, max-age=60, stale-while-revalidate=30');header('X-Runner3-Speed: HIT');header('X-Runner3-Speed-Version: 1.0.0');header('Content-Length: '.$runner3_size);}if($runner3_method==='HEAD')exit;readfile($runner3_file);exit;
