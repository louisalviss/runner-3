...
if(res.headers.get('x-r3-reader-ios-clean')!=='v119')throw new Error('clean ios header missing');
if(res.headers.get('x-r3-reader-legacy-chain')!=='bypassed-v119')throw new Error('legacy bypass header missing');
...
if(!html.includes("center-toggle-v119"))throw new Error('v119 center-toggle gesture missing');
if(!html.includes('__r3IosPageNextV117')||!html.includes('nav.edge.done')||!html.includes("r3DirectPageV117('next')"))throw new Error('v119 edge navigation owner missing');
if(!html.includes('.topbar{top:20px!important}')||!html.includes('top:max(28px,env(safe-area-inset-top,0px))')||!html.includes('width:28vw')||!html.includes('opacity:0!important')||!html.includes('bottom:calc(132px + env(safe-area-inset-bottom,0px))'))throw new Error('v119 product geometry missing');
if(!html.includes('__r3CleanAudioFollowV119')||!html.includes('r3-audio-follow-v119')||!html.includes('audio.timing.ready')||!html.includes('setTimeout(runFollowLoop,90)'))throw new Error('v119 event-driven audio follow missing');
if(html.includes('setInterval(runFollowLoop'))throw new Error('v119 idle audio polling leaked');
if(html.includes('nav.direct.done'))throw new Error('v119 old visible-nav telemetry leaked');
