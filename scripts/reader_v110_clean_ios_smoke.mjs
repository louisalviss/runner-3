...
if(res.headers.get('x-r3-reader-ios-clean')!=='v114')throw new Error('clean ios header missing');
if(res.headers.get('x-r3-reader-legacy-chain')!=='bypassed-v114')throw new Error('legacy bypass header missing');
...
if(!html.includes("rendition.hooks.content.register"))throw new Error('v114 content hook missing');
if(!html.includes("frame.hook.bound"))throw new Error('v114 frame trace missing');
if(!html.includes(".chrome{opacity:1!important}"))throw new Error('v114 always-visible chrome missing');
