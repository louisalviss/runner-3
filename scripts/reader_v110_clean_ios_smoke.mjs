...
if(res.headers.get('x-r3-reader-ios-clean')!=='v121')throw new Error('clean ios header missing');
if(res.headers.get('x-r3-reader-legacy-chain')!=='bypassed-v121')throw new Error('legacy bypass header missing');
...
if(!html.includes("center-toggle-v121"))throw new Error('v121 center-toggle gesture missing');
if(!html.includes("frame.getBoundingClientRect().width")||!html.includes("chrome.center.toggle"))throw new Error('v121 visible-frame center geometry missing');
if(!html.includes('__r3IosPageNextV117')||!html.includes('nav.edge.done')||!html.includes("r3DirectPageV117('next')"))throw new Error('v121 edge navigation owner missing');
if(!html.includes('.topbar{top:28px!important}')||!html.includes('top:max(44px,calc(env(safe-area-inset-top,0px) + 16px))')||!html.includes('width:28vw')||!html.includes('opacity:0!important')||!html.includes('bottom:calc(132px + env(safe-area-inset-bottom,0px))'))throw new Error('v121 product geometry missing');
if(!html.includes('__r3CleanAudioFollowV120')||!html.includes('r3-audio-follow-v120')||!html.includes('sentenceRange(index)')||!html.includes('audio.timing.ready')||!html.includes('setTimeout(runFollowLoop,90)'))throw new Error('v121 event-driven audio follow missing');
if(html.includes('setInterval(runFollowLoop'))throw new Error('v121 idle audio polling leaked');
if(html.includes('nav.direct.done'))throw new Error('v121 old visible-nav telemetry leaked');
