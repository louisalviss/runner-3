...
if(res.headers.get('x-r3-reader-ios-clean')!=='v118')throw new Error('clean ios header missing');
if(res.headers.get('x-r3-reader-legacy-chain')!=='bypassed-v118')throw new Error('legacy bypass header missing');
...
if(!html.includes('__r3IosPageNextV117')||!html.includes('nav.edge.done')||!html.includes("r3DirectPageV117('next')"))throw new Error('v118 edge navigation owner missing');
if(!html.includes('.topbar{top:12px!important}')||!html.includes('width:34vw')||!html.includes('opacity:0!important')||!html.includes('bottom:calc(132px + env(safe-area-inset-bottom,0px))'))throw new Error('v118 broad invisible edge zones missing');
if(html.includes('nav.direct.done'))throw new Error('v118 old visible-nav telemetry leaked');
