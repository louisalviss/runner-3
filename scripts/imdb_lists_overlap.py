...
# Current IMDb list UI labels: List order / Alphabetical / Date added.
# Asc + desc of list_order guarantees both ends of the custom list; alpha/date_added
# are additional independent cuts for long watchlists that cap one rendered view at 250.
SORTS = [
    "list_order,asc",
    "list_order,desc",
    "alpha,asc",
...
def load_view(page,max_rounds=24):
    history=[]; stable=0
...
def scan_union(page, base_url, stem):
    ...
    for sort_value in SORTS:
        ...
        print(stem,sort_value,'view',len(rows),'union',len(union),'reported',reported,flush=True)
    ...
    return {"source_url":base_url,"reported_total":reported,"captured_unique":len(rows),"complete":reported is not None and len(rows)>=reported,"rows":rows}
