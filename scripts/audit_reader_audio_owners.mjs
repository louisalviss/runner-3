...
const names = fs.readdirSync(root)
  .filter((name) => /^artifact-library-reader-v(?:1[1-9]|2\d|3[01])-[^/]+-entry\.js$/.test(name))
  .sort((a, b) => {
...
console.log(`READER_AUDIO_OWNER_AUDIT=PASS files=${names.length} matches=${rows.length}`);
