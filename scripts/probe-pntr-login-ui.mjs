...
try {
  ...
  const buttons = await page.locator('button,input[type=submit]').evaluateAll(els => els.map(e => ({text:(e.innerText||e.value||'').replace(/\s+/g,' ').trim(), type:e.getAttribute('type'), name:e.getAttribute('name')})).filter(x=>x.text));
  ...
  const forms = await page.locator('form').evaluateAll(els => els.map(e => ({action:e.getAttribute('action'), method:e.getAttribute('method')||'get'})));
