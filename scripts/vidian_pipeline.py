...
UA = "Mozilla/5.0 (compatible; runner-3/VidianPipeline/2.0)"
SEEDS = {
    "review", "tin-tuc", "tom-tat", "tac-gia", "tieu-diem-nhan-vat",
    "trich-dan-kinh-dien", "chi-dao-sang-tac", "bang-xep-hang",
}
UNTRUSTED = {
...
def tokens(x):
    return re.findall(r"[0-9A-Za-zÀ-ỹĐđ]+", (x or "").lower())
...
def dependency_parser():
    global _DEPENDENCY_PARSE
    if _DEPENDENCY_PARSE is None:
        from underthesea import dependency_parse
        _DEPENDENCY_PARSE = dependency_parse
    return _DEPENDENCY_PARSE
...
def parser_smoke():
    ...
    samples = [
        "Tôi yêu Việt Nam.",
        "Sinh viên đọc sách ở thư viện.",
        "Tối nay Hà Nội có mưa lớn.",
    ]
    parser = dependency_parser()
    batch = parser(samples)
    if len(batch) != len(samples):
        raise SystemExit(f"dependency parser batch mismatch: {len(batch)}!={len(samples)}")
    parsed = []
    for dep in batch:
        if not dep or not all(isinstance(x, (tuple, list)) and len(x) == 3 for x in dep):
            raise SystemExit("dependency parser returned invalid structure")
        if not any(int(head) == 0 or str(rel) == "root" for _, head, rel in dep):
            raise SystemExit("dependency parser returned no root")
        parsed.append(len(dep))
    print(json.dumps({
        "parser_smoke": "ok",
...
def canon(h):
    ...
    u = urljoin(BASE, h)
    p = urlparse(u)
    if p.netloc not in {"vidian.vn", "www.vidian.vn"}:
        return None
...
def category(h):
    p = urlparse(urljoin(BASE, h or ""))
    m = re.search(r"/danh-muc/([^/?#]+)/?", p.path)
...
def discover_categories():
    ...
    try:
        ...
        print("DISCOVER_WARN", repr(e), flush=True)
    return sorted(out)
...
def last_page(slug):
    for u in (f"{BASE}/danh-muc/{slug}/1/", f"{BASE}/danh-muc/{slug}/"):
        try:
            ...
            soup = BeautifulSoup(r.text, "html.parser")
            nums = {1}
            rx = re.compile(rf"/danh-muc/{re.escape(slug)}/(\d+)/?")
            for a in soup.find_all("a", href=True):
    ...
    try:
        ...
        for a in soup.find_all("a", href=True):
            ...
            if c:
                ...
                d[c] = max(d.get(c, ""), t, key=len)
        return slug, page, d, None
    except Exception as e:
        return slug, page, {}, f"{type(e).__name__}:{e}"
...
def inventory(outdir):
    ...
    with ThreadPoolExecutor(max_workers=12) as ex:
        ...
        for i, f in enumerate(as_completed(fs), 1):
            c, p, d, err = f.result()
            if err:
                failures.append({"category": c, "page": p, "error": err})
            for u, t in d.items():
                by[u] = max(by.get(u, ""), t, key=len)
            if i % 50 == 0 or i == len(jobs):
                print("LISTING", i, "/", len(jobs), flush=True)
...
def is_marker(x):
    ...
    return (
        low in {"nguồn", "từ khóa"}
        or low.startswith("nguồn:")
        or low.startswith("từ khóa:")
        or any(low.startswith(m) for m in MARKERS)
    )
...
def extract(root):
    ...
    for node in h1.xpath("following::text()"):
        ...
        if is_marker(x):
            flush()
            break
        if x.lower() in {"video", "rank", "tìm kiếm", "chat", "user"}:
            continue
        pid = id(parent)
...
def sentence_split(text):
    xs = [
        clean(x)
        for x in re.split(r'(?<=[.!?…])\s+(?=[A-ZÀ-ỸĐ0-9“"(\[])', text)
        if len(clean(x)) >= 25
    ]
...
def phrase_matches(text, phrases):
    ...
    for p in phrases:
        q = " ".join(tokens(p))
        if q and re.search(r"(?:^| )" + re.escape(q) + r"(?: |$)", low):
            out.append(p)
    return sorted(out)
...
def numeric(text):
    c = Counter(re.findall(
        r"(?<!\w)(?:\d{1,4}(?:[.,]\d+)?%?|20\d{2}[-/]\d{1,2}[-/]\d{1,2}|\d+\s*(?:năm|tháng|ngày|tuổi|cấp|tầng|lần|người|vạn|triệu|tỷ))(?!\w)",
        text.lower(),
    ))
...
def frame_from_dep(sent, idx, dep):
    nodes = [{"word": str(w), "head": int(h), "rel": str(r)} for w, h, r in dep]
    roots = [i for i, n in enumerate(nodes, 1) if n["rel"] == "root" or n["head"] == 0]
    root_i = roots[0] if roots else None
    pred = nodes[root_i - 1]["word"] if root_i else ""
    subjects = []
    objects = []
    comps = []
    for n in nodes:
        if n["rel"].startswith(("nsubj", "csubj")):
            subjects.append(n["word"])
        elif n["rel"] in {"obj", "iobj"}:
            objects.append(n["word"])
        elif n["rel"].startswith("obl") or n["rel"] in {"advmod", "xcomp", "ccomp"}:
            comps.append(n["word"])
    ...
    return {
        "sentence_index": idx,
        "source_sentence_sha256": hashlib.sha256(clean(sent).encode()).hexdigest(),
        "predicate_root": pred,
        "subjects": subjects[:8],
        "objects": objects[:8],
        "obliques_and_complements": comps[:12],
        "negation": phrase_matches(sent, NEG),
        "modality_uncertainty": phrase_matches(sent, MODAL),
        "causal_cues": phrase_matches(sent, CAUSAL),
        "temporal_cues": phrase_matches(sent, TEMP),
        "condition_cues": phrase_matches(sent, COND),
        "comparison_cues": phrase_matches(sent, COMP),
        "attribution_cues": phrase_matches(sent, ATTR),
        "entities": entities(sent),
        "numeric_signals": numeric(sent),
        "residual_lexicon": [{"term": t, "count": c} for t, c in sorted(terms.items())],
        "dependency_edges": edges,
        "word_count": len(tokens(sent)),
    }
...
def parse_error_frame(sent, idx, error):
    return {
        "sentence_index": idx,
        "source_sentence_sha256": hashlib.sha256(clean(sent).encode()).hexdigest(),
        "parse_error": f"{type(error).__name__}:{str(error)[:160]}",
        "entities": entities(sent),
        "numeric_signals": numeric(sent),
        "residual_lexicon": [
            {"term": t, "count": c}
            for t, c in Counter(tokens(sent)).most_common(80)
        ],
    }
...
def parse_many(texts, group_size=512):
    ...
    def parse_range(lo, hi):
        ...
        try:
            ...
            if len(parsed) != len(batch):
                raise ValueError(f"batch-size-mismatch:{len(parsed)}!={len(batch)}")
            for j, dep in enumerate(parsed, lo):
                results[j] = dep
    ...
    for lo in range(0, len(texts), group_size):
        hi = min(lo + group_size, len(texts))
        parse_range(lo, hi)
        print("PARSE_BATCH", hi, "/", len(texts), flush=True)
    return results
...
def prepare_article(row):
    ...
    try:
        r = session().get(u, timeout=(6, 25), allow_redirects=True)
        o["http_status"] = r.status_code
        o["html_sha256"] = hashlib.sha256(r.content).hexdigest()
        if not r.ok:
            o["status"] = f"http-{r.status_code}"
            return o
        root = html.fromstring(r.content, base_url=r.url)
        paras = extract(root)
        full = clean(" ".join(paras))
        o["clean_body_sha256"] = hashlib.sha256(full.encode()).hexdigest()
        o["paragraph_count"] = len(paras)
        title = (
            root.xpath('//meta[@property="og:title"]/@content')
            or root.xpath("//h1[1]//text()")
            or root.xpath("//title/text()")
        )
        o["title"] = clean(title[0]) if title else ""
        o["_paragraphs"] = paras
        o["status"] = "fetched"
    except Exception as e:
        o["status"] = f"error:{type(e).__name__}:{str(e)[:120]}"
    finally:
        o["fetch_elapsed_sec"] = round(time.time() - started, 3)
    return o
...
def semanticize_prepared(prepared, parse_group_size=512):
    ...
    for ai, rec in enumerate(prepared):
        ...
        if rec.get("status") != "fetched" or not paras:
            continue
        layout = []
        sentence_index = 0
        for pi, paragraph in enumerate(paras):
            ...
            for sent in sentence_split(paragraph):
                flat_index = len(flat_texts)
                flat_texts.append(sent)
                paragraph_items.append((flat_index, sentence_index, sent))
                sentence_index += 1
            layout.append(paragraph_items)
        article_layouts[ai] = layout
        rec["sentence_count"] = sentence_index

    parse_started = time.time()
    parsed_results = parse_many(flat_texts, group_size=parse_group_size)
    parse_elapsed = time.time() - parse_started

    for ai, rec in enumerate(prepared):
        layout = article_layouts.get(ai)
        if layout is None:
            rec["elapsed_sec"] = rec.get("fetch_elapsed_sec", 0)
            rec["fetched_at_utc"] = datetime.now(timezone.utc).isoformat()
            continue
        sec = {"section_index": 0, "heading": "", "paragraphs": []}
        for pi, paragraph_items in enumerate(layout):
            prec = {"paragraph_index": pi, "sentences": []}
            for flat_index, sentence_index, sent in paragraph_items:
                result = parsed_results[flat_index]
                if isinstance(result, Exception):
                    frame = parse_error_frame(sent, sentence_index, result)
                    rec["parse_failed_sentences"] += 1
                else:
                    try:
                        frame = frame_from_dep(sent, sentence_index, result)
                        rec["parse_success_sentences"] += 1
                    except Exception as exc:
                        frame = parse_error_frame(sent, sentence_index, exc)
                        rec["parse_failed_sentences"] += 1
                prec["sentences"].append(frame)
            sec["paragraphs"].append(prec)
        rec["sections"] = [sec]
        rec["status"] = "ok" if not rec["parse_failed_sentences"] else "partial-parse"
        rec["elapsed_sec"] = round(rec.get("fetch_elapsed_sec", 0) + parse_elapsed, 3)
        rec["fetched_at_utc"] = datetime.now(timezone.utc).isoformat()
...
def chunk(inventory_path, outdir, index, count, fetch_workers=8, parse_group_size=512):
    ...
    rows = [r for r in inv["rows"] if r.get("trusted")]
    selected = [r for i, r in enumerate(rows) if i % count == index]
    out = Path(outdir)
    out.mkdir(parents=True, exist_ok=True)
    p = out / f"vidian_semantic_frame_chunk_{index:02d}.jsonl"

    fetch_started = time.time()
    prepared = [None] * len(selected)
    with ThreadPoolExecutor(max_workers=min(fetch_workers, max(1, len(selected)))) as ex:
        future_to_i = {ex.submit(prepare_article, row): i for i, row in enumerate(selected)}
        done = 0
        for future in as_completed(future_to_i):
            ...
            if done % 25 == 0 or done == len(selected):
                print("PREFETCH", index, done, "/", len(selected), flush=True)

    fetch_elapsed = time.time() - fetch_started
    semantic_started = time.time()
    prepared = semanticize_prepared(prepared, parse_group_size=parse_group_size)
    semantic_elapsed = time.time() - semantic_started

    ok = bad = sent = parsed = parse_failed = 0
    with p.open("w", encoding="utf-8", buffering=1) as f:
        for rec in prepared:
            f.write(json.dumps(rec, ensure_ascii=False) + "\n")
            sent += rec.get("sentence_count", 0)
            parsed += rec.get("parse_success_sentences", 0)
            parse_failed += rec.get("parse_failed_sentences", 0)
            if rec["status"] in {"ok", "partial-parse"}:
                ok += 1
            else:
                bad += 1

    parse_rate = parsed / sent if sent else 0
    summary = {
        "chunk": index,
        "chunks": count,
        "rows": len(selected),
        ...
        "fetch_elapsed_sec": round(fetch_elapsed, 3),
        "semantic_elapsed_sec": round(semantic_elapsed, 3),
        "completed_utc": datetime.now(timezone.utc).isoformat(),
    }
    (out / f"chunk_{index:02d}_summary.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    print(json.dumps(summary, ensure_ascii=False), flush=True)

    if sent == 0 or parse_rate < 0.95:
        raise SystemExit(f"dependency parse rate too low: {parse_rate:.4f}")
    if bad:
        raise SystemExit(f"{bad} article fetch/extraction failures")


def merge(indir, outdir, inventory_path, count):
    inv = json.loads(Path(inventory_path).read_text())
    expected = len([r for r in inv["rows"] if r.get("trusted")])
    src = Path(indir)
    files = sorted(src.rglob("vidian_semantic_frame_chunk_*.jsonl"))
    if len(files) != count:
        raise SystemExit(f"expected {count} chunk files, found {len(files)}")
    rows = []
    for p in files:
    ...
    for p in files:
        ...
        with gzip.open(dest, "wb", compresslevel=9) as g:
    ...
    total = sum(r.get("sentence_count", 0) for r in rows)
    parsed = sum(r.get("parse_success_sentences", 0) for r in rows)
    failed_parse = sum(r.get("parse_failed_sentences", 0) for r in rows)
    summary = {
        "schema": "semantic-reconstruction-frames-v3",
        "expected_trusted_urls": expected,
        "records": len(rows),
        "unique_urls": len(set(urls)),
        "chunk_files": len(files),
        "ok_articles": sum(r.get("status") == "ok" for r in rows),
        "partial_parse_articles": sum(r.get("status") == "partial-parse" for r in rows),
        "failed_articles": sum(r.get("status") not in {"ok", "partial-parse"} for r in rows),
        "total_sentences": total,
        "parse_success_sentences": parsed,
        "parse_failed_sentences": failed_parse,
    ...
    if len(rows) != expected or len(set(urls)) != expected:
        raise SystemExit(3)
    if summary["failed_articles"]:
        raise SystemExit(4)
    if total == 0 or summary["parse_rate"] < 0.95:
        raise SystemExit(5)
...
def main():
    ...
    a.add_argument("--out", default="vidian_inventory")

    a = sub.add_parser("chunk")
    a.add_argument("--inventory", required=True)
    a.add_argument("--out", required=True)
    a.add_argument("--index", type=int, required=True)
    a.add_argument("--count", type=int, default=32)
    a.add_argument("--fetch-workers", type=int, default=8)
    a.add_argument("--parse-group-size", type=int, default=512)
    ...
    if x.cmd == "smoke":
        parser_smoke()
    elif x.cmd == "inventory":
        inventory(x.out)
    elif x.cmd == "chunk":
        chunk(x.inventory, x.out, x.index, x.count, x.fetch_workers, x.parse_group_size)
    else:
        merge(x.input, x.out, x.inventory, x.count)
