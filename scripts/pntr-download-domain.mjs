...
let stage='start';
let state={verified:false,stage,checkedAt:new Date().toISOString()};
function checkpoint(next,extra={}){
  stage=next; state={...state,...extra,stage,checkedAt:new Date().toISOString()};
  fs.writeFileSync('/tmp/pntr-download/status.json',JSON.stringify(state,null,2)+'\n');
}
function fail(err){
  checkpoint('failed',{error:String(err?.message||err).slice(0,800)});
...
checkpoint('browser_start',{pagesTarget,oldDomainUntouched:'runner3wp.pntr.dev'});
...
if(subs.status===401){
  ...
  if(anon.status>=300) throw new Error('pntr_anonymous_auth_failed:'+anon.status);
  subs=await api('/api/subdomains');
}
if(subs.status!==200) throw new Error('pntr_guest_session_failed:'+subs.status);
checkpoint('guest_session_ready');
...
checkpoint('domain_catalog_ready');
...
for(const name of candidates){
  ...
  if([200,201].includes(cr.status)){sub=cr.body?.subdomain||cr.body?.data||cr.body?.result||cr.body;chosen=name;break;}
}
...
checkpoint('pntr_registered',{hostname:fqdn,subdomainIdPresent:true,directUrl:`https://${fqdn}/%40LinkFilesBot.apk`});
...
const cfList=await fetch(cfBase,{headers:cfHeaders}).then(r=>r.json());
if(!cfList.success) throw new Error('cf_pages_domain_list_failed:'+JSON.stringify(cfList.errors||[]));
...
if(!current){
  ...
  const j=await r.json(); if(!j.success) throw new Error('cf_pages_domain_create_failed:'+JSON.stringify(j.errors||[])); current=j.result;
}
checkpoint('pages_domain_associated',{customDomainStatus:current?.status||null});

const rec=await api(`/api/subdomains/${encodeURIComponent(subId)}/records`,'POST',{record_type:'CNAME',record_value:pagesTarget});
if(![200,201].includes(rec.status)) throw new Error('pntr_cname_failed:'+rec.status+':'+JSON.stringify(rec.body).slice(0,500));
checkpoint('cname_created');
await browser.close();

let domainStatus=current?.status||null,http=0,disp='',ctype='',size=0,sha='';
for(let i=0;i<24;i++){
  ...
  try{
    ...
    if(http===200){const b=Buffer.from(await r.arrayBuffer());size=b.length;sha=crypto.createHash('sha256').update(b).digest('hex');if(size===expectedSize&&sha===expectedSha&&/^attachment/i.test(disp))break;}
  }catch{}
  await new Promise(r=>setTimeout(r,5000));
}
const verified=http===200&&size===expectedSize&&sha===expectedSha&&/^attachment/i.test(disp);
checkpoint(verified?'verified':'verify_timeout',{customDomainStatus:domainStatus,httpCode:http,contentDisposition:disp,contentType:ctype,size,sha256:sha,verified,workerHostnameHidden:verified});
