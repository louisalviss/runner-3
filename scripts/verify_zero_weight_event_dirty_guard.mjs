...
if (!dirtyReasonAllowsPriorityMaterialization('explicit_interest_ingest')) throw new Error('explicit D1 fallback must get guarded priority repair');
...
async function proveStaleDirtyRepair(reason) {
  let cleanWrites = 0;
      ...
      const q={
        ...
        async first(){
          if (sql.includes('SELECT status,run_id,detail,updated_at FROM workflow_state')) {
            return {status:'dirty',run_id:null,detail:JSON.stringify({reason}),updated_at:'2026-09-11 17:01:13'};
          }
          if (sql.includes("FROM content_scores WHERE score_type='personal_relevance'")) return {ok:1};
  ...
  if (!result.ok || result.status !== 'clean' || result.recomputed !== false || result.zero_weight_dirty_repaired !== true) {
    throw new Error(`${reason} repair failed: ${JSON.stringify(result)}`);
  }
  if (cleanWrites !== 1 || expensiveQueries !== 0) throw new Error(`${reason} repair was not bounded: clean=${cleanWrites} expensive=${expensiveQueries}`);
  return result;
}

await proveStaleDirtyRepair('event_batch');
await proveStaleDirtyRepair('append_only_learning_ingest');
console.log(JSON.stringify({ok:true,shown_affects_profile:false,stale_dirty_repair:['event_batch','append_only_learning_ingest'],explicit_fallback_priority:true}));
