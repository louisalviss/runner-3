...
async function verify(){stage('verify_wordpress');state.siteUrl=nativeUrl(cfg.app_name);state.dashboardUrl=dashboard(cfg.app_name);for(let i=0;i<18;i++){const home=await ctx.request.get(state.siteUrl,{timeout:10000,failOnStatusCode:false}).catch(()=>null);state.httpCode=home?.status()??null;const api=await ctx.request.get(new URL('/wp-json/',state.siteUrl).href,{timeout:10000,failOnStatusCode:false}).catch(()=>null);state.wpApiReachable=!!api&&api.status()>=200&&api.status()<400;await page.goto(state.dashboardUrl,{waitUntil:'domcontentloaded',timeout:60000}).catch(()=>null);state.wpAdminReachable=await page.getByText(/WordPress Admin/i).first().isVisible().catch(()=>false);save();if(state.httpCode&&state.httpCode<500&&state.wpAdminReachable&&state.wpApiReachable)return;await page.waitForTimeout(5000);}throw new Error('wordpress_readiness_failed');}
