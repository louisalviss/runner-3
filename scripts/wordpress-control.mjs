...
async function frontend(path='/') {
  ...
  const r = await fetch(base + normalized, {
    method: 'GET',
    headers: { Accept: 'text/html,application/xhtml+xml' },
    redirect: 'follow'
  });
  const html = await r.text();
  if (!r.ok) throw new Error(`Frontend GET ${normalized} failed ${r.status}`);
...
switch (action) {
  ...
  case 'verify_robots': {
    ...
    const page = await frontend(pagePath);
    result = { path: pagePath, httpStatus: page.status, ...inspectRobots(page.html) };
    break;
  }
