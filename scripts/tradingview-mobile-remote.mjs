...
async function attach(p){
  ...
  cdp.on('Page.screencastFrame', async ev => {
    ...
    const msg = JSON.stringify({t:'frame',data:ev.data,w:frameW,h:frameH,url:page.url()});
...
*{box-sizing:border-box}body{margin:0;background:#0b0e11;color:white;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif}#top{position:sticky;top:0;z-index:4;background:#111827;padding:8px;display:flex;gap:7px;align-items:center}button,input{font-size:16px;border:0;border-radius:9px;padding:10px}button{background:#2563eb;color:white;font-weight:700}#save{background:#16a34a}.grow{flex:1}#screen{display:block;width:100%;height:auto;touch-action:none;background:#111}#url{font-size:11px;color:#9ca3af;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;padding:4px 8px;background:#111827}#kbd{position:sticky;bottom:0;z-index:4;background:#111827;padding:8px;display:flex;gap:6px}#text{min-width:0;background:white;color:black}.note{font-size:12px;color:#cbd5e1;padding:5px 8px}</style><div id=top><button id=back>‹</button><button id=reload>↻</button><button id=google class=grow>TradingView login</button><button id=save>SAVE</button></div><div id=url></div><img id=screen><div class=note>Tap trực tiếp lên màn hình. Khi cần gõ, nhập ở ô dưới rồi bấm Send. Sau khi thấy TradingView đã đăng nhập, bấm SAVE.</div><div id=kbd><input id=text class=grow placeholder="Nhập text…"><button id=send>Send</button><button id=enter>Enter</button></div><script>
const s=document.getElementById('screen'),u=document.getElementById('url'),q=document.getElementById('text');let W=430,H=850;const ws=new WebSocket((location.protocol==='https:'?'wss://':'ws://')+location.host+'/ws');ws.onmessage=e=>{const m=JSON.parse(e.data);if(m.t==='frame'){W=m.w;H=m.h;s.src='data:image/jpeg;base64,'+m.data;u.textContent=m.url||''}if(m.t==='saved')alert('Đã lưu session TradingView')};function send(o){if(ws.readyState===1)ws.send(JSON.stringify(o))}s.onclick=e=>{const r=s.getBoundingClientRect();send({t:'tap',x:(e.clientX-r.left)*W/r.width,y:(e.clientY-r.top)*H/r.height})};document.getElementById('back').onclick=()=>send({t:'back'});document.getElementById('reload').onclick=()=>send({t:'reload'});document.getElementById('google').onclick=()=>send({t:'login'});document.getElementById('send').onclick=()=>{send({t:'text',v:q.value});q.value=''};document.getElementById('enter').onclick=()=>send({t:'enter'});document.getElementById('save').onclick=()=>send({t:'save'});q.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();document.getElementById('send').click();send({t:'enter'})}});
</script>`;
