...
const out = '/tmp/runner3-speed-clean-room-a.json';
...
async function browserRuns(browser,n=7){
  ...
  for(let i=0;i<n;i++){
    ...
    const resp=await page.goto(base+'/',{waitUntil:'load',timeout:60000}); await sleep(600);
...
function lighthouseRuns(n=3){
  const rows=[];
  for(let i=0;i<n;i++){
    const file=`/tmp/lh-clean-a-${i}.json`;
...
const report={status:'starting',site:slug,url:base,state:'A_NO_PLUGIN',pluginAbsent:false,cloudflare:null,curl:null,browser:null,lighthouse:null,checkedAt:null};
try{
  ...
  report.curl=curlRuns(15); report.browser=await browserRuns(browser,7); report.lighthouse=lighthouseRuns(3);
