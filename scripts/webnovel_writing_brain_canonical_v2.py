...
def build(base,outdir,max_neighbors=12):
    ...
    for rid,midx in enumerate(clusters,1):
        ...
        mx=max([edge_sim.get(tuple(sorted((a,b))),0.0) for ai,a in enumerate(midx) for b in midx[ai+1:]] or [1.0]);rulehash=h('|'.join([topic,direction,norm(text)]))
        con.execute('INSERT INTO canonical_rules VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',(rid,topic,direction,kind,title,text,cconf(ms),len(ms),src['vidian'],src['moxing'],len(src),int(len(src)>1),json.dumps(all_atoms,ensure_ascii=False),rep['id'],rulehash,'conservative-vietnamese-tfidf-v2',round(mx,6)))
