...
try{
 ...
 const state=await page.evaluate(()=>({clean:document.documentElement.dataset.r3CleanIos||'',debug:window.__r3CleanIosV112||null,boot:window.__r3IosMinimalBootV112||null,locations:window.__r3IosLocationsV112||null,legacy35:Boolean(document.querySelector('script[data-r3-audio-continuity-v35]')),legacy82:Boolean(document.querySelector('script[data-r3-stable-shell-runtime-v82]')),frameText:String(document.querySelector('#viewer iframe')?.contentDocument?.body?.innerText||'').trim().length,controls:document.body.classList.contains('controls')}));
 if(state.clean!=='v112'||state.legacy35||state.legacy82||state.frameText<80||state.boot?.phase!=='done'||state.locations?.disabled!==true)throw new Error('V112_BAD_RUNTIME '+JSON.stringify(state));
