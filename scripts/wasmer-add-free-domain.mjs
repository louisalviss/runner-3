...
const clean=s=>String(s||'').replace(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/ig,'EMAIL_REDACTED').slice(0,3200);
...
async function snapshot(p,label){
  const controls=await p.locator('button,input,a,[role=dialog]').evaluateAll(xs=>xs.map(x=>({
    tag:x.tagName.toLowerCase(),
    text:(x.innerText||x.textContent||'').replace(/\s+/g,' ').trim().slice(0,140),
    type:x.getAttribute('type'),
    name:x.getAttribute('name'),
    placeholder:x.getAttribute('placeholder'),
    value:(x.tagName==='INPUT'&&x.getAttribute('type')!=='password')?x.value:null,
  })).filter(x=>x.text||x.placeholder||x.name).slice(0,60)).catch(()=>[]);
...
try{
  ...
  if(!t.toLowerCase().includes(targetDomain.toLowerCase())){
    ...
    const candidates=p.locator('input[type=text],input[name*=domain i],input[placeholder*=domain i],input[type=url]');
    ...
    // Prefer the Add button in the open dialog; fall back to another visible Add/Save/Continue.
    ...
    if(!submit){
      const all=p.locator('button').filter({hasText:/^\s*(Add|Save|Continue)\s*$/i});
      const n=await all.count();
      for(let i=n-1;i>=0;i--){const el=all.nth(i);if(await el.isVisible().catch(()=>false)){submit=el;break;}}
    }
  ...
  out.verified=(/valid configuration|verified|active|ready/i.test(t) && !/pending|waiting|not verified|unverified|invalid configuration/i.test(t));
  ...
  out.detail=await snapshot(p,'final');
  save();
