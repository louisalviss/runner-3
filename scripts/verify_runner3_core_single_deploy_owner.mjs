...
if (!/artifact-library-reader-v35-continuity-single-owner-entry\.js/.test(owner)) throw new Error('canonical Reader v35 composition missing');
if (!/v34\+v35:ahead-prefetch\+range-follow\+single-audio-owner/.test(owner)) throw new Error('canonical Reader v35 proof gate missing');
if (!/singleAudioListenerOwner:true/.test(owner)) throw new Error('canonical Reader v35 single-audio-owner gate missing');
if (!/__r3AudioLegacyV11Suppressed=true/.test(owner)) throw new Error('canonical Reader v33 core legacy suppression gate missing');
console.log(`RUNNER3_CORE_SINGLE_DEPLOY_OWNER=PASS owner=${allowed} workflows=${files.length} detection=step-aware preview-aware canonical=v35`);
