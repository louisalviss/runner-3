...
def _chunks(text,max_chars=160):
    text=(text or '').strip()
    if len(text)<=max_chars:return [text]
    units=[x for x in SENT_SPLIT.split(text) if x]
    expanded=[]
    for u in units:
        if len(u)<=max_chars: expanded.append(u);continue
        clauses=[x for x in CLAUSE_SPLIT.split(u) if x]
        expanded.extend(clauses)
    out=[];cur=''
    for u in expanded:
        if len(u)>max_chars:
            if cur:out.append(cur);cur=''
            for i in range(0,len(u),max_chars):out.append(u[i:i+max_chars])
        elif not cur:cur=u
...
def translate(inp,out,shard,shards,batch_size,max_new_tokens):
    ...
    with target.open('w',encoding='utf-8') as w:
        for start in range(0,len(picked),batch_size):
            ...
            for bi,x in enumerate(batch):
                cs=_chunks(x['text_zh']); per.append(len(cs)); chunked_rows+=len(cs)>1;total_chunks+=len(cs)
                for c in cs:flat.append(c);owners.append(bi)
            flat_out=_translate_batch(tok,translator,flat,max_new_tokens)
            grouped=[[] for _ in batch]
            for bi,vi in zip(owners,flat_out):grouped[bi].append(vi)
            for x,parts,nchunks in zip(batch,grouped,per):
                vi=re.sub(r'\s+',' ',' '.join(parts)).strip()
                rec={
                  ...
                  'model_license':MODEL_LICENSE,'runtime':RUNTIME,'translation_chunks':nchunks,
                }
    ...
    manifest={'schema':'moxing-vi-translation-shard-v1.2','shard':shard,'shards':shards,'rows':len(picked),'chunked_rows':chunked_rows,'translation_chunks':total_chunks,'model_id':MODEL_ID,'model_revision':MODEL_REVISION,'model_license':MODEL_LICENSE,'runtime':RUNTIME,'long_passage_policy':'split at Chinese sentence/clause punctuation around 160 chars before translation'}
    target.with_suffix('.manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding='utf-8')
    print(json.dumps(manifest,ensure_ascii=False))

def quality(path):
    total=empty=zh_heavy=length_outlier=0; chars=zhchars=chunked=0
    for _,r in rows(path):
        total+=1; t=r.get('text_vi','').strip(); empty+=not bool(t); c=len(t); z=len(ZH.findall(t)); chars+=c; zhchars+=z;chunked+=int(r.get('translation_chunks',1)>1)
        if c and z/c>0.25: zh_heavy+=1
        if c>1200: length_outlier+=1
    return {'rows':total,'empty':empty,'zh_heavy_rows':zh_heavy,'zh_char_ratio':round(zhchars/max(chars,1),6),'length_outlier_rows':length_outlier,'chunked_rows':chunked}
