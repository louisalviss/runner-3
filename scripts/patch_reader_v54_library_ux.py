...
# ---------------------------------------------------------------------------
# Main Library UI: replace only libraryPage(). Server routing/upload stays intact.
# ---------------------------------------------------------------------------
...
end_marker = 'function injectIframeSwipe(html) {'
end = simple.find(end_marker, start)
if start < 0 or end < 0:
    raise SystemExit('v54: simple libraryPage boundaries missing')
simple = simple[:start] + LIBRARY_PAGE + '\n\n' + simple[end:]

# ---------------------------------------------------------------------------
# Reader: persist exact progress and extract EPUB metadata/cover into IndexedDB.
# ---------------------------------------------------------------------------
meta_helpers = r'''
...
if 'R3_LIBRARY_META_DB_V54' not in reader:
    reader = replace_once(reader, marker, marker + '\n' + meta_helpers, 'v54 reader metadata helpers')

# Persist percentage on every relocation; preserve the canonical CFI key.
pattern = re.compile(r"rendition\.on\('relocated',loc=>\{const cfi=loc\?\.start\?\.cfi;if\(cfi\)persist\(keys\.position,cfi\);const pct=Number\.isFinite\(loc\?\.start\?\.percentage\)\?Math\.round\(loc\.start\.percentage\*100\):null;\$\('position'\)\.textContent=pct===null\?'Đã lưu vị trí':pct\+'% · đã lưu';setTimeout\(bindEpubContents,0\);\}\);")
replacement = "rendition.on('relocated',loc=>{const cfi=loc?.start?.cfi;if(cfi)persist(keys.position,cfi);const pct=Number.isFinite(loc?.start?.percentage)?Math.max(0,Math.min(100,Math.round(loc.start.percentage*100))):null;try{localStorage.setItem(R3_READER_PROGRESS_PREFIX_V54+key,JSON.stringify({percent:pct,cfi:cfi||'',updatedAt:Date.now()}));}catch{}$('position').textContent=pct===null?'Đã lưu vị trí':pct+'% · đã lưu';setTimeout(bindEpubContents,0);});"
reader, count = pattern.subn(replacement, reader, count=1)
if count != 1 and 'R3_READER_PROGRESS_PREFIX_V54+key' not in reader:
    raise SystemExit(f'v54 relocated progress handler: expected 1 match, got {count}')

if 'r3PersistBookMetaV54(book,key)' not in reader:
    reader = replace_once(reader, '      book=window.ePub(buffer);', "      book=window.ePub(buffer);\n      setTimeout(()=>r3PersistBookMetaV54(book,key),0);", 'v54 persist epub metadata')

# Live Library: canonical title resolver and metadata/progress-aware cards.
old_title = "  const r3Humanize=s=>String(s||'').replace(/[-_]+/g,' ').replace(/\\b\\w/g,c=>c.toUpperCase());\n  const r3TitleFor=b=>r3Humanize((b&&b.scope)||((b&&b.key||'').split('/')[2])||(((b&&b.key||'').split('/').pop()||'Book').replace(/\\.epub$/i,'')));"
new_title = "  const r3TitleFor=b=>r3TitleForBookV54(b);"
if old_title in reader:
    reader = replace_once(reader, old_title, new_title, 'v54 live title resolver')
elif new_title not in reader:
    raise SystemExit('v54 live title resolver marker missing')
...
new_render = r'''  function r3RenderLiveLibrary(){
...
# Ensure current live panel also hydrates cached cover metadata even before list refresh.
old_open = "    r3LoadLiveLibrary();\n  }"
new_open = "    r3HydrateMetaCacheV54().then(()=>{if(r3LiveLibraryVisible())r3RenderLiveLibrary();});r3LoadLiveLibrary();\n  }"
if old_open in reader:
    reader = replace_once(reader, old_open, new_open, 'v54 live open meta hydration')
...
# Machine-translated DCC labels were the source of the confusing titles.
