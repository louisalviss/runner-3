...
const helpers=String.raw`function cssBlocks(css) {
  ...
  while(i<css.length){
    ...
    const prelude=css.slice(i,open).trim(); let depth=1,j=open+1;
    for(;j<css.length&&depth;j++){ if(css[j]==='{') depth++; else if(css[j]==='}') depth--; }
    if(depth) break;
    out.push({prelude,body:css.slice(open+1,j-1)}); i=j;
  }
  return out;
}
...
function filterCriticalCss(css) {
  ...
  for(const block of cssBlocks(css)){
    ...
    if(/^@(?:media|supports)\b/.test(p)){
      const inner=filterCriticalCss(block.body); if(inner.trim()) out+=`${p}{${inner}}\n`;
...
const oldCritical=String.raw`  const baseCritical = criticalCss.slice(rootStart, editionStart).trim();
...
const newCritical=String.raw`  const criticalSubset = filterCriticalCss(criticalCss);
  if (!criticalSubset || Buffer.byteLength(criticalSubset) > 9000) {
...
const newDeferred=String.raw`  const deferredMarkup = deferred.map(({ tag }) => tag.trim()).join('\n');
