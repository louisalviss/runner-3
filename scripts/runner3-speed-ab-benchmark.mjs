...
async function install(wp){
  ...
  const btn=wp.locator('#install-plugin-submit').first();
  if(!(await btn.count()))throw new Error('plugin_upload_submit_missing');
  ...
  const r=await row(wp);
  if(!(await r.count()))throw new Error('runner3_speed_install_missing');
  if(expectedVersion){
    const text=await r.innerText();
    if(!text.includes(`Version ${expectedVersion}`))throw new Error(`plugin_version_mismatch:expected_${expectedVersion}`);
  }
}
...
async function browserRuns(browser,n=7){
  ...
  for(let i=0;i<n;i++){
    ...
    await page.addInitScript(()=>{
      window.__runner3Lcp=0;window.__runner3Cls=0;
      new PerformanceObserver(list=>{for(const e of list.getEntries())window.__runner3Lcp=Math.max(window.__runner3Lcp,e.startTime||0);}).observe({type:'largest-contentful-paint',buffered:true});
      new PerformanceObserver(list=>{for(const e of list.getEntries())if(!e.hadRecentInput)window.__runner3Cls+=e.value||0;}).observe({type:'layout-shift',buffered:true});
    });
    ...
    const perf=await page.evaluate(()=>{
      ...
      return{ttfb:nav?.responseStart||0,domContentLoaded:nav?.domContentLoadedEventEnd||0,load:nav?.loadEventEnd||0,fcp:fcp?.startTime||0,lcp:window.__runner3Lcp||0,cls:window.__runner3Cls||0};
    });
    const h=resp?.headers()||{};
    rows.push({status:resp?.status()||0,wallMs:Date.now()-started,speed:h['x-runner3-speed']||null,speedVersion:h['x-runner3-speed-version']||null,...perf});
    await ctx.close();
  }
...
async function request(url){
  ...
  return {status:r.status,speed:r.headers.get('x-runner3-speed'),speedVersion:r.headers.get('x-runner3-speed-version'),body:await r.text()};
}

async function validateSafety(wp, warm){
  const safety={installed:true,active:true,hit:false,queryBypass:false,apiBypass:false,adminBypass:false,off:false,productionUnchanged:false};
  if(warm.speed!=='HIT'||warm.speedVersion!==expectedVersion) throw new Error(`safety_hit_failed:${warm.speed||'none'}:${warm.speedVersion||'none'}`);
  safety.hit=true;

  const query=await request(`${base}/?runner3_probe=${Date.now()}`);
  if(query.status!==200||query.speed==='HIT') throw new Error(`query_bypass_failed:${query.status}:${query.speed||'none'}`);
  safety.queryBypass=true;

  const api=await request(`${base}/wp-json/`);
  if(api.status<200||api.status>=500||api.speed==='HIT') throw new Error(`api_bypass_failed:${api.status}:${api.speed||'none'}`);
  safety.apiBypass=true;

  await wp.goto(`${base}/wp-admin/plugins.php`,{waitUntil:'domcontentloaded',timeout:60000});
  if(!/wp-admin\/plugins\.php/.test(wp.url())) throw new Error('admin_bypass_failed');
  safety.adminBypass=true;

  await toggle(wp,false);
  const off=await request(`${base}/`);
  if(off.status!==200||off.speed==='HIT'||off.body.length<512) throw new Error(`off_bypass_failed:${off.status}:${off.speed||'none'}`);
  safety.off=true;

  const prod=await fetch(`https://runner3wp.pntr.dev/?__runner3_speed_probe=${Date.now()}`);
  const prodHtml=await prod.text();
  if(!prod.ok||(prod.headers.get('x-edge-snapshot')||'').toUpperCase()!=='HIT'||(prod.headers.get('x-edge-cache-policy')||'').toLowerCase()!=='snapshot-direct'||!/__runner3\/r2-image\/offset-demo-01-w(?:360|480|640)\.webp/.test(prodHtml)) throw new Error('production_v2_changed');
  safety.productionUnchanged=true;
  return safety;
}
...
let wp;
const report={status:'starting',site:slug,url:base,pluginVersion:expectedVersion,cloudflare:false,baseline:null,optimized:null,optimizedWarm:null,delta:null,safety:null,checkedAt:null};

try{
  await login(page);
  wp=await adminPage(adminCtx,page);
  await install(wp);
  await activate(wp);
  await toggle(wp,false);
  await sleep(1500);
  ...
  report.optimizedWarm={speed:warmResponse.headers.get('x-runner3-speed'),speedVersion:warmResponse.headers.get('x-runner3-speed-version'),bytes:warmBody.length,layoutStabilizer:warmBody.includes('runner3-inspiro-layout-stabilizer'),fontPreloadCount:(warmBody.match(/rel=["']preload["'][^>]+as=["']font["']/gi)||[]).length};
  ...
  report.safety=await validateSafety(wp,report.optimizedWarm);
  report.status='ready';
  report.checkedAt=new Date().toISOString();
  fs.writeFileSync(out,JSON.stringify(report,null,2)+'\n');
  console.log('RUNNER3_SPEED_AB_RESULT');
  ...
  if(wp)await toggle(wp,false).catch(()=>{});
