...
async function install(wp){
  let r=await row(wp); if(await r.count()) return;
  ...
  const btn=wp.locator('#install-plugin-submit').first(); if(!(await btn.count()))throw new Error('plugin_upload_submit_missing'); await btn.click({force:true}); await sleep(1800);
  ...
  if(await replace.count()){const href=await replace.getAttribute('href').catch(()=>null);if(href)await wp.goto(new URL(href,`${base}/wp-admin/`).href,{waitUntil:'domcontentloaded',timeout:60000});else await replace.click({force:true}).catch(()=>{});await sleep(1500);}
  r=await row(wp); if(!(await r.count()))throw new Error('runner3_speed_install_missing');
}
...
async function browserRuns(browser,n=7){
  ...
  for(let i=0;i<n;i++){
    ...
    await page.addInitScript(()=>{window.__runner3Lcp=0;new PerformanceObserver(list=>{for(const e of list.getEntries())window.__runner3Lcp=Math.max(window.__runner3Lcp,e.startTime||0);}).observe({type:'largest-contentful-paint',buffered:true});});
    const started=Date.now(); const resp=await page.goto(base+'/',{waitUntil:'load',timeout:60000}); await sleep(600);
    const perf=await page.evaluate(()=>{const nav=performance.getEntriesByType('navigation')[0];const fcp=performance.getEntriesByName('first-contentful-paint')[0];return{ttfb:nav?.responseStart||0,domContentLoaded:nav?.domContentLoadedEventEnd||0,load:nav?.loadEventEnd||0,fcp:fcp?.startTime||0,lcp:window.__runner3Lcp||0};});
    rows.push({status:resp?.status()||0,wallMs:Date.now()-started,...perf}); await ctx.close();
  }
  const pick=k=>round(median(rows.map(x=>Number(x[k]))));
  return {runs:rows,medianTtfbMs:pick('ttfb'),medianFcpMs:pick('fcp'),medianLcpMs:pick('lcp'),medianDclMs:pick('domContentLoaded'),medianLoadMs:pick('load'),medianWallMs:pick('wallMs')};
}
...
const report={status:'starting',site:slug,url:base,cloudflare:false,baseline:null,optimized:null,delta:null,checkedAt:null};
try{
  ...
  report.delta={curlTtfbMs:round(o.curl.medianTtfbMs-b.curl.medianTtfbMs),curlTtfbPct:round((o.curl.medianTtfbMs/b.curl.medianTtfbMs-1)*100),browserTtfbMs:round(o.browser.medianTtfbMs-b.browser.medianTtfbMs),browserTtfbPct:round((o.browser.medianTtfbMs/b.browser.medianTtfbMs-1)*100),browserFcpMs:round(o.browser.medianFcpMs-b.browser.medianFcpMs),browserLcpMs:round(o.browser.medianLcpMs-b.browser.medianLcpMs),lighthouseScore:round(o.lighthouse.medianScore-b.lighthouse.medianScore,0),lighthouseFcpMs:round(o.lighthouse.medianFcpMs-b.lighthouse.medianFcpMs),lighthouseLcpMs:round(o.lighthouse.medianLcpMs-b.lighthouse.medianLcpMs),lighthouseServerResponseMs:round(o.lighthouse.medianServerResponseMs-b.lighthouse.medianServerResponseMs)};
