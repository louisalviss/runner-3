...
await page.addInitScript(() => {
  ...
  window.__r3V34LiveBootAudit = audit;
});
...
try {
  ...
  if (runtime !== 'v34-continuous-range-sync') throw new Error(`LIVE_RUNTIME_WRONG:${runtime}`);
  if (proof !== 'v33+v34:ahead-prefetch+range-follow+manual-sync') throw new Error(`LIVE_PROOF_WRONG:${proof}`);
  ...
  const result = await page.evaluate(async () => {
    ...
    return {
      ...
      continuityMarker: Boolean(document.querySelector('script[data-r3-audio-continuity-v34="1"]')),
      flags: {
  ...
  if (!result.ownerMarker || !result.coreRuntimeMarker || !result.continuityMarker) throw new Error('V34_DOM_MARKERS_MISSING');
  ...
  if (result.continuityOwner !== 'reader-audio-continuity-v34') throw new Error(`CONTINUITY_OWNER_WRONG:${result.continuityOwner}`);
  ...
  if (beforeHref !== afterHref || beforeCfi !== afterCfi) throw new Error('V34_PEEK_MUTATED_READER');

  console.log(JSON.stringify({
    ok: true,
    runtime,
    proof,
    coreSingleListenerOwner: true,
    continuityOwner: result.continuityOwner,
  ...
  console.log('READER_AUDIO_CORE_V34_LIVE_BOOT_E2E=PASS');
} finally {
  await browser.close();
}
