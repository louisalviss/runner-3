...
const safe = {
  ...
  theme: 'Inspiro', starterPlugin: 'Inspiro Starter Sites',
  themeActive: false, starterPluginActive: false,
  importAttempted: false, importComplete: false,
  pages: 0, posts: 0, media: 0, homepageHttp: null,
  steps: [], detail: null, updatedAt: new Date().toISOString()
};
...
async function enterAdmin(ctx, page) {
  ...
  for (let n = 1; n <= 3; n++) {
    ...
    if (await settings.count() && await settings.isVisible().catch(() => false)) {
      await settings.click({ noWaitAfter: true }).catch(() => {});
      await page.waitForTimeout(1000);
      const wordpress = page.getByText(/^WordPress$/i).first();
      if (await wordpress.count() && await wordpress.isVisible().catch(() => false)) {
        await wordpress.click({ noWaitAfter: true }).catch(() => {});
        await page.waitForTimeout(1200);
      }
      found = await tryAdminControl(ctx, page);
      if (found) return found;
    }
...
async function themeCard(wp) {
  const cards = wp.locator('.theme');
  for (let i = 0; i < await cards.count(); i++) {
    const c = cards.nth(i);
    const t = await c.innerText().catch(() => '');
    if (/\bInspiro\b/i.test(t) && !/Inspiro Blocks/i.test(t)) return c;
  }
  return null;
}

async function ensureInspiroTheme(wp) {
  ...
  let card = await themeCard(wp);
  if (!card) {
    await wp.goto(`${base}/wp-admin/theme-install.php?search=Inspiro`, { waitUntil: 'domcontentloaded', timeout: 60000 });
    ...
    for (let i = 0; i < await cards.count(); i++) {
      const c = cards.nth(i);
      const t = await c.innerText().catch(() => '');
      if (/\bInspiro\b/i.test(t) && !/Inspiro Blocks/i.test(t)) { installCard = c; break; }
    }
    if (!installCard) throw new Error(`inspiro_search_result_missing:${(await body(wp)).slice(0,500)}`);
    const install = installCard.locator('a,button').filter({ hasText: /^Install$/i }).first();
    if (!(await install.count())) throw new Error('inspiro_install_control_missing');
    await install.click({ noWaitAfter: true });
    const end = Date.now() + 90000;
    let installed = false;
    while (Date.now() < end) {
      const txt = await installCard.innerText().catch(() => '');
      if (/Activate|Installed/i.test(txt)) { installed = true; break; }
      await wp.waitForTimeout(1500);
    }
    if (!installed) throw new Error('inspiro_install_timeout');
  }

  await wp.goto(`${base}/wp-admin/themes.php`, { waitUntil: 'domcontentloaded', timeout: 60000 });
  await wp.waitForTimeout(1000);
  card = await themeCard(wp);
  if (!card) throw new Error('inspiro_theme_not_found_after_repo_install');
  let txt = await card.innerText().catch(() => '');
  if (!/Active:/i.test(txt) && !/Customize/i.test(txt)) {
    const activate = card.locator('a,button').filter({ hasText: /^Activate$/i }).first();
    if (!(await activate.count())) throw new Error('inspiro_activate_missing');
    await activate.click({ noWaitAfter: true });
    await wp.waitForTimeout(3500);
  }
  await wp.goto(`${base}/wp-admin/themes.php`, { waitUntil: 'domcontentloaded', timeout: 60000 });
  await wp.waitForTimeout(800);
  safe.themeActive = /Inspiro/i.test(await wp.locator('.theme.active').first().innerText().catch(() => ''));
  save();
  if (!safe.themeActive) throw new Error('inspiro_activation_unconfirmed');
}

async function pluginCard(wp) {
  const cards = wp.locator('.plugin-card');
  for (let i = 0; i < await cards.count(); i++) {
    const c = cards.nth(i);
    if (/Inspiro Starter Sites/i.test(await c.innerText().catch(() => ''))) return c;
  }
  return null;
}

async function ensureStarterPlugin(wp) {
  step('install_inspiro_starter_sites');
  await wp.goto(`${base}/wp-admin/plugins.php`, { waitUntil: 'domcontentloaded', timeout: 60000 });
  await wp.waitForTimeout(800);
  let rows = wp.locator('tr.active');
  for (let i = 0; i < await rows.count(); i++) {
    if (/Inspiro Starter Sites/i.test(await rows.nth(i).innerText().catch(() => ''))) {
      safe.starterPluginActive = true; save(); return;
    }
  }

  await wp.goto(`${base}/wp-admin/plugin-install.php?s=Inspiro%20Starter%20Sites&tab=search&type=term`, { waitUntil: 'domcontentloaded', timeout: 60000 });
  await wp.waitForTimeout(1800);
  const card = await pluginCard(wp);
  if (!card) throw new Error(`starter_plugin_search_missing:${(await body(wp)).slice(0,500)}`);
  let action = card.locator('a,button').filter({ hasText: /^Install Now$/i }).first();
  if (await action.count()) {
    await action.click({ noWaitAfter: true });
    const end = Date.now() + 90000;
    while (Date.now() < end) {
      const txt = await card.innerText().catch(() => '');
      if (/Activate/i.test(txt)) break;
      await wp.waitForTimeout(1500);
    }
  }
  action = card.locator('a,button').filter({ hasText: /^Activate$/i }).first();
  if (await action.count()) {
    await action.click({ noWaitAfter: true });
    await wp.waitForTimeout(3500);
  }
  ...
  rows = wp.locator('tr.active');
  for (let i = 0; i < await rows.count(); i++) {
    if (/Inspiro Starter Sites/i.test(await rows.nth(i).innerText().catch(() => ''))) {
      safe.starterPluginActive = true; save(); return;
    }
  }
  throw new Error('starter_plugin_activation_unconfirmed');
}

async function importDemo(wp) {
  step('open_demo_importer');
  safe.importAttempted = true; save();
  await wp.goto(`${base}/wp-admin/admin.php?page=inspiro-demo`, { waitUntil: 'domcontentloaded', timeout: 60000 });
  await wp.waitForTimeout(2500);
  let txt = await body(wp);
  console.log('importer start', txt.slice(0, 700));
  if (/Default Starter Content Detected/i.test(txt)) {
    ...
    if (await remove.count() && await remove.isVisible().catch(() => false)) {
      await remove.click({ noWaitAfter: true }).catch(() => {});
  ...
  if (!(await card.count())) throw new Error(`business_portfolio_demo_missing:${(await body(wp)).slice(0,700)}`);
  const pick = card.locator('button,a').filter({ hasText: /Select|Import Demo|Import|Choose/i }).first();
  if (await pick.count() && await pick.isVisible().catch(() => false)) await pick.click({ noWaitAfter: true });
  else await card.click({ noWaitAfter: true });
  await wp.waitForTimeout(1800);

  for (let round = 0; round < 60; round++) {
    txt = await body(wp);
    if (/Demo Imported|Import Complete|Successfully Imported|Your site is ready|import has finished/i.test(txt)) {
      safe.importComplete = true; save(); break;
    }
    const patterns = [
...
async function restCount(path) {
  ...
  if (!r.ok) return { count: 0, status: r.status };
  const j = await r.json();
  return { count: Array.isArray(j) ? j.length : 0, status: r.status };
}
...
try {
  ...
  await ensureInspiroTheme(wp);
  await ensureStarterPlugin(wp);
  await importDemo(wp);
  step('verify_seed');
  const [pages, posts, media, home] = await Promise.all([
    restCount('/wp-json/wp/v2/pages?per_page=100&_fields=id'),
    restCount('/wp-json/wp/v2/posts?per_page=100&_fields=id'),
    restCount('/wp-json/wp/v2/media?per_page=100&_fields=id'),
