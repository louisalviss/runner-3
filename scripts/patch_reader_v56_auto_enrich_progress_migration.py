...
browser_helpers = r'''
  ...
  function zipJoinV56(base,href){let value=String(href||'').split(String.fromCharCode(92)).join('/').split('#')[0].split('?')[0];try{value=decodeURIComponent(value)}catch{}const parts=String(base||'').split('/').filter(Boolean);for(const part of value.split('/')){if(!part||part==='.')continue;if(part==='..')parts.pop();else parts.push(part)}return parts.join('/')}
