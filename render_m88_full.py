...
def ordered_market_names(markets):
    ...
    unknown = [name for name, lines in markets.items() if lines and name not in MARKET_ORDER]
...
def match_details(m, scope, idx):
    ...
    if scope == "live":
        status = (
            ...
            f'<span class="score">{esc(home_score(m.get("home_score")))} : {esc(away_score(m.get("away_score")))}</span>'
            f'{live_clock(m)}'
        )
    else:
...
<details class="match" id="match-{scope}-{idx}">
  ...
  <div class="detail">
    <div class="detail-head">
      <button class="close" type="button" data-close-details>‹</button>
      <div><strong>{esc(m.get("home"))} vs {esc(m.get("away"))}</strong><small>{status} · {esc(league_name(m))}</small></div>
    </div>
...
def league_sections(rows, scope):
    ...
    def flush(name, ms, start):
        ...
        cards = []
        for x in ms:
            cards.append(match_details(x, scope, start))
            start += 1
    ...
    for m in rows:
        ...
        if current is None:
            current = ln
        if ln != current:
            block, idx = flush(current, items, idx)
            out.append(block)
            current, items = ln, []
        items.append(m)
    block, idx = flush(current, items, idx)
...
<style>
:root{{--navy:#172137;--league:#566783;--peach:#faebe7;--red:#c92816;--yellow:#f1c927;--blue:#5e8be2;--ink:#273852;--muted:#778397}}
...
.detail{{background:#fae8e4}}.detail-head{{min-height:85px;background:#182235;color:#fff;display:grid;grid-template-columns:34px 1fr;align-items:center;padding:10px 12px}}.detail-head .close{{border:0;background:none;color:#fff;font-size:33px;text-align:left;padding:0;cursor:pointer}}.detail-head strong{{display:block;font-size:15px;line-height:1.35}}.detail-head small{{display:flex;align-items:center;gap:6px;flex-wrap:wrap;color:#c0c7d2;margin-top:6px;font-size:11px;text-transform:uppercase}}.markets{{padding:12px 7px 18px}}.market{{margin-bottom:16px}}.market h3{{margin:0 0 4px;text-align:center;font-size:14px;font-weight:500;color:#2c3e5b}}.odds-grid{{display:grid;gap:2px;margin-bottom:2px}}.grid2{{grid-template-columns:repeat(2,1fr)}}.grid3{{grid-template-columns:repeat(3,1fr)}}.odd{{min-height:58px;background:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:5px;text-align:center}}.odd span{{font-size:12px;color:#7b8595;line-height:1.2}}.odd b{{font-size:16px;font-weight:500;color:#253750;margin-top:3px}}.empty{{padding:28px 12px;text-align:center;color:#7e8998}}
@media(max-width:390px){{.match-name{{font-size:14px}}.odd b{{font-size:15px}}}}
</style></head>
