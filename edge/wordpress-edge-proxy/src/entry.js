...
async function publicHtmlResponse(request, env, ctx) {
  let response = await worker.fetch(request, env, ctx); response = await decorateFrontend(request, response); const headers = new Headers(response.headers); headers.delete('Content-Length');
  if (!cacheableHtmlResponse(response)) { headers.set('Cache-Control', 'no-store'); headers.delete('Cloudflare-CDN-Cache-Control'); headers.delete('Cache-Tag'); headers.set('X-Edge-Cache-Policy', 'bypass'); }
  else { headers.set('Cache-Control', 'public, max-age=60, stale-while-revalidate=30, stale-if-error=600'); headers.set('Cloudflare-CDN-Cache-Control', 'public, max-age=86400, stale-while-revalidate=3600, stale-if-error=604800'); headers.set('Cache-Tag', HTML_CACHE_TAG); headers.set('X-Edge-Cache-Policy', 'workers-caching'); }
...
async function verifyPurgeSignature(request, env, rawBody) {
  const timestampText = request.headers.get('X-Runner3-Timestamp') || ''; const signatureText = request.headers.get('X-Runner3-Signature') || ''; const secret = String(env.RUNNER3_CACHE_PURGE_SECRET || '');
  const timestamp = Number(timestampText); if (!secret) return { ok: false, error: 'auth_unconfigured' };
...
async function warmAndVerify(ctx, incoming, path) {
  try {
    const url = new URL(path, incoming.origin); const first = await ctx.exports.PublicHtml.fetch(new Request(url, { method: 'GET' })); await first.arrayBuffer();
    const second = await ctx.exports.PublicHtml.fetch(new Request(url, { method: 'GET' })); await second.arrayBuffer(); const status = String(second.headers.get('CF-Cache-Status') || '').toUpperCase(); const age = second.headers.get('Age');
...
async function handlePurge(request, env, ctx) {
  ...
  return jsonResponse({ ok: true, purged: Boolean(purge && purge.ok), cache_verified: cacheVerified, signing_algorithm: auth.algorithm, tag: HTML_CACHE_TAG, reason: typeof body.reason === 'string' ? body.reason.slice(0, 120) : 'wordpress-change', warmed });
}
