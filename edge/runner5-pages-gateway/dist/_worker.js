// Runner5 production Pages gateway.
// Promoted from V100 Candidate 6: preserves theme JavaScript and fixes the
// Inspiro first-paint header/layout mismatch structurally.
...
export default {
  async fetch(request,env){
    ...
    if(url.pathname==='/__runner5/pages-gateway/health'){
      return Response.json({ok:true,gateway:'runner5-restore-gateway',downstream:'runner5-restore-proxy',candidate:6},{headers:{'Cache-Control':'no-store','X-Robots-Tag':'noindex,nofollow'}});
    }
    if(!env.EDGE||typeof env.EDGE.fetch!=='function') return new Response('Runner5 Pages gateway downstream unavailable',{status:503,headers:{'Cache-Control':'no-store'}});
    ...
    const cssP=htmlCandidate?env.EDGE.fetch(new Request(new URL('/__edge/runner5.css',url.origin),{headers:{Accept:'text/css','User-Agent':'Runner5Candidate6Production/1.0'}})).catch(()=>null):Promise.resolve(null);
