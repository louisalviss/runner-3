...
def _prepare_source(c):
    ...
    work = Path(tempfile.mkdtemp(prefix="us15-source-"))
    ...
    for key, spec in manifest["files"].items():
        ...
        if key == "profile":
            ...
            if missing:
                raise RuntimeError(f"15m universe not subset of 30m source: {missing}")
            profile["universe"] = universe
            profile["primary_exclude"] = []
            profile["status"] = "PREREGISTERED_15M_PROMOTION_SOURCE"
            profile["selection"] = {
                "source": "30m preregistered symbol-gate survivors",
    ...
    out_manifest["selection"] = {
        "rule": "EXACT_30M_SYMBOL_GATE_SURVIVORS_ONLY",
        "symbols": universe,
        "count": len(universe),
        "gates_changed": False,
...
def _patch_staged_lineage(c):
    scope = c["scope"]
    work = Path(tempfile.mkdtemp(prefix="us15-lineage-"))
    mp = work / "manifest.json"
    ...
    profile["lineage"] = {
        ...
        "experiment": "US_EQUITIES_15M_PROMOTION_FROM_30M_SURVIVORS",
        "holdout_start": c["holdout_start"],
        "parameter_changes": "TIMEFRAME_ONLY_30M_TO_15M",
        "direction": "LONG_ONLY",
        "session": "PRESERVED_FROM_CANONICAL",
        "universe_rule": "EXACT_12_PREREGISTERED_30M_SURVIVORS",
        "gates": "UNCHANGED_FROM_30M_SYMBOL_PROMOTION_GATES",
    }
    profile["status"] = "PREREGISTERED_15M_PROMOTION_TEST"
    wj(pp, profile)
    ...
    manifest["discipline"] = {
        ...
        "next_step": "ONLY_15M_SURVIVORS_MAY_ENTER_5M",
    }
    wj(mp, manifest)
...
def stage(a):
    ...
    temp = Path(tempfile.mkdtemp(prefix="us15-config-")) / "config.json"
    cc = dict(c)
...
def evaluate(a):
    c = cfg(a.config)
    rc = _redirect_checkpoint(c, base.evaluate, a)
    return rc
