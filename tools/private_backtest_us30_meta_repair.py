...
SCOPE = "bt-super-rsi-us-equities-30m-probe-v1"
...
def main():
    work = Path(tempfile.mkdtemp(prefix="us30-meta-repair-"))
    ...
    repairs = list(manifest.get("transport_repairs", []))
    if not any(x.get("repair") == "META_DUKASCOPY_LEGACY_ALIAS" for x in repairs if isinstance(x, dict)):
        repairs.append({"repair": "META_DUKASCOPY_LEGACY_ALIAS", "mapping": "META->FB.US/USD", "strategy_changes": "NONE"})
    manifest["transport_repairs"] = repairs
    mp.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    core.upload_artifact(PROJECT, SCOPE, "manifest.json", mp, "application/json; charset=utf-8")
    print(json.dumps({"scope": SCOPE, "helper": helper_name, "helper_sha256": manifest["files"]["helper"]["sha256"], "repair": "META->FB.US/USD", "strategy_changes": "NONE"}))
