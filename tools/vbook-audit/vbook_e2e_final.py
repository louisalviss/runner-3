...
def discover(i,limit=4):
        ...
        if len(found)<limit:
            for z in aa[:24]:
                ...
                if fn and b.main_script(i,fn) and sig(i,fn) is not None:
                    ...
                    tr.append([fn,y.get('ok'),y.get('kind'),len(yy),trace_sample(yy) if yy and not any(looks_item(q) for q in yy) else None,str(y.get('err',''))[:500],str(y.get('inner') or '')[:500]])
                    add(yy)
    ...
    if sr and len(found)<limit:
        ...
        for q in qs:
            x=invoke(i,sr,q); aa=listdata(x); tr.append(['search:'+q,x.get('ok'),x.get('kind'),len(aa),trace_sample(aa) if aa and not any(looks_item(z) for z in aa) else None,str(x.get('err',''))[:500],str(x.get('inner') or '')[:500]])
            add(aa)
            if len(found)>=limit: break
    return found,tr
