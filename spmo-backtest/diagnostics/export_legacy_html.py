...
raw=r.text
needle='PowerShares S&P 500 Momentum Portfolio'.lower()
count=0
for di,doc in enumerate(re.findall(r'<DOCUMENT>(.*?)</DOCUMENT>',raw,re.S|re.I)):
    if needle not in doc.lower(): continue
...
if count==0: raise RuntimeError('No legacy document containing SPMO found')
