...
models=[('inclusionai/ling-3.0-flash','ling3_paid_none'),('qwen/qwen3.7-flash','qwen37_flash_none')]
out=Path('vbth_bench/results_v4'); out.mkdir(parents=True,exist_ok=True)
def run(x):
 mid,tag=x; p={'model':mid,'messages':[{'role':'system','content':system},{'role':'user','content':source}],'temperature':0.15,'max_tokens':2200,'usage':{'include':True},'reasoning':{'effort':'none'}}
 req=urllib.request.Request(API,data=json.dumps(p).encode(),headers={'Authorization':'Bearer '+KEY,'Content-Type':'application/json','HTTP-Referer':'https://github.com/louisalviss/runner-3','X-Title':'VBTH final cheap rerun'})
