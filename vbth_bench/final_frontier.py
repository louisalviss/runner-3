...
models=[('deepseek/deepseek-v3.2','deepseek_v32'),('qwen/qwen3.5-plus-20260420','qwen35_plus'),('google/gemini-2.5-flash','gemini25_flash'),('minimax/minimax-m3','minimax_m3')]
out=Path('vbth_bench/results_v5'); out.mkdir(parents=True,exist_ok=True)
def run(x):
 mid,tag=x; p={'model':mid,'messages':[{'role':'system','content':system},{'role':'user','content':source}],'temperature':0.15,'max_tokens':2200,'usage':{'include':True}}
 if mid in {'deepseek/deepseek-v3.2','qwen/qwen3.5-plus-20260420','google/gemini-2.5-flash','minimax/minimax-m3'}: p['reasoning']={'effort':'none'}
 req=urllib.request.Request(API,data=json.dumps(p).encode(),headers={'Authorization':'Bearer '+KEY,'Content-Type':'application/json','HTTP-Referer':'https://github.com/louisalviss/runner-3','X-Title':'VBTH cost-quality frontier'})
