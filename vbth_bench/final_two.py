...
def between(a,b): s=raw.index(a); e=raw.index(b,s); return raw[s:e].strip()
...
models=[('deepseek/deepseek-v3.2','deepseek_v32'),('minimax/minimax-m3','minimax_m3')]
out=Path('vbth_bench/results_v6'); out.mkdir(parents=True,exist_ok=True)
def run(x):
 mid,tag=x; p={'model':mid,'messages':[{'role':'system','content':system},{'role':'user','content':source}],'temperature':0.15,'max_tokens':2200,'usage':{'include':True},'reasoning':{'enabled':False}}
 req=urllib.request.Request(API,data=json.dumps(p).encode(),headers={'Authorization':'Bearer '+KEY,'Content-Type':'application/json','HTTP-Referer':'https://github.com/louisalviss/runner-3','X-Title':'VBTH final two'})
 t=time.time(); text=''; u={}; actual=''; err=''; status='error'
 try:
  with urllib.request.urlopen(req,timeout=50) as r: obj=json.loads(r.read())
