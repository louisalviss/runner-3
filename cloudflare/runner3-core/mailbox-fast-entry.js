...
export async function handleMailboxFast(request, env, url = new URL(request.url)) {
  if (request.method === "GET" && url.pathname === "/mailbox/health") {
    return Response.json(
      { ok: true, service: "runner3-core-mailbox", version: "mailbox-fast-v1" },
...
export default {
  async fetch(request, env) {
    const response = await handleMailboxFast(request, env);
