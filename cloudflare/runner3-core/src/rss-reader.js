...
async function load(){list.textContent='';status.className='';status.textContent='Đang tải…';try{const j=await api('/reader/rss/library');auth.style.display='none';status.textContent=j.count+' bài';for(const a of j.articles){const d=document.createElement('div');d.className='item';const h=document.createElement('a');h.href='/rss/article/'+encodeURIComponent(a.article_id);h.textContent=a.title;const m=document.createElement('div');m.className='meta';m.textContent=[a.source_name,a.published_at,'fetch:'+a.fetch_status,'vi:'+a.translation_status].filter(Boolean).join(' · ');d.append(h,m);list.append(d)}}catch(e){auth.style.display='flex';status.className='bad';status.textContent=e.message==='UNAUTHORIZED'?'Reader token không hợp lệ':e.message}}load();
</script></body></html>`;
}
...
async function view(kind){err.textContent='';body.textContent='Đang tải…';try{const j=await api('/reader/rss/articles/'+encodeURIComponent(id)+'/'+kind);body.textContent=j.artifact.body||''}catch(e){body.textContent='';err.textContent=e.message==='UNAUTHORIZED'?'Reader token không hợp lệ':e.message}}
async function init(){try{const j=await api('/reader/rss/articles/'+encodeURIComponent(id));const a=j.article;title.textContent=a.title;meta.textContent=[a.source_name,a.published_at,'fetch:'+a.fetch_status,'vi:'+a.translation_status].filter(Boolean).join(' · ');const preferred=(a.source_language==='vi'||a.translation_status==='published'||a.translation_status==='native_vi')?'vi':'original';await view(preferred)}catch(e){err.textContent=e.message==='MISSING_READER_TOKEN'?'Mở lại RSS Library để đăng nhập':e.message}}
