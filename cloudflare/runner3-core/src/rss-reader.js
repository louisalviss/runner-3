...
body{font:17px system-ui,-apple-system,sans-serif;max-width:820px;margin:24px auto;padding:0 18px 70px;line-height:1.68;color:#111}h1{font-size:32px;line-height:1.15}.top{display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin:14px 0}button{font:inherit;padding:9px 12px;border:1px solid #ccc;border-radius:10px;background:#f5f5f5;cursor:pointer}pre{white-space:pre-wrap;font:inherit;margin:24px 0}.meta{font-size:13px;opacity:.65}.bad{color:#a00}a{color:inherit}.source{margin-left:auto;font-size:14px}.images{display:grid;gap:18px;margin-top:28px}.images figure{margin:0}.images img{display:block;width:100%;height:auto;max-height:700px;object-fit:contain;border-radius:10px;background:#f2f2f2}.images figcaption{font-size:13px;opacity:.68;margin-top:6px}@media(max-width:600px){body{margin:18px auto}h1{font-size:28px}.source{margin-left:0;width:100%}}
</style></head><body><p><a href="/rss/library">← Library</a></p><h1 id="title">RSS Article</h1><div class="top"><button id="vi">Tiếng Việt</button><button id="original">Original</button><a class="source" id="source" target="_blank" rel="noopener noreferrer">Bài gốc ↗</a></div><p class="meta" id="meta"></p><p class="bad" id="error"></p><pre id="body"></pre><section class="images" id="images"></section><script>
const KEY='rssReaderToken',id=${encoded},body=document.querySelector('#body'),images=document.querySelector('#images'),err=document.querySelector('#error'),meta=document.querySelector('#meta'),title=document.querySelector('#title'),source=document.querySelector('#source');
function decodeText(value){const el=document.createElement('textarea');el.innerHTML=String(value??'');return el.value}
...
async function view(kind){err.textContent='';body.textContent='Đang tải…';images.textContent='';try{const j=await api('/reader/rss/articles/'+encodeURIComponent(id)+'/'+kind);body.textContent=decodeText(j.artifact.body||'');renderImages(j.artifact.images)}catch(e){body.textContent='';err.textContent=e.message==='UNAUTHORIZED'?'Reader token không hợp lệ':e.message}}
