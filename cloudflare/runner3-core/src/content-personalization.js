...
const RECOMMENDATION_ID_CHUNK = 50;
function idChunks(values,size=RECOMMENDATION_ID_CHUNK){const out=[];for(let i=0;i<values.length;i+=size)out.push(values.slice(i,i+size));return out;}
...
async function scoreShownItems(env, sourceRenderId){
  ...
  for(const batch of idChunks(ids)){
    ...
    const part=await env.DB.prepare(`SELECT f.item_id,f.feature_type,${canonicalKeySql} AS feature_key,f.weight AS feature_weight,f.confidence AS feature_confidence,
      ...
      WHERE f.item_id IN (${placeholders}) AND f.feature_type IN ('topic','concept','mechanism')`).bind(...batch).all();
    featureRows.push(...(part.results||[]));
  }
  const features={results:featureRows};
