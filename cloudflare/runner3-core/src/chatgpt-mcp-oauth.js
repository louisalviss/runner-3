...
function oauthJson(payload, status = 200) { return jsonResponse(payload, status, { pragma: "no-cache" }); }
...
async function cleanupOauth(env) {
  ...
  await env.DB.batch([
    env.DB.prepare("DELETE FROM mcp_oauth_codes WHERE expires_at < ? OR (used_at IS NOT NULL AND used_at < ?)").bind(now - 300, now - 300),
...
async function validateChatGptClient(clientId, redirectUri) {
  ...
  if (client.protocol !== "https:" || client.hostname !== "chatgpt.com" || !client.pathname.startsWith("/oauth/") || !client.pathname.endsWith("/client.json")) throw new Error("invalid_client");
...
function protectedResourceMetadata() {
  return { resource: MCP_RESOURCE, authorization_servers: [ORIGIN], scopes_supported: [OAUTH_SCOPE], bearer_methods_supported: ["header"] };
}
...
async function handleAuthorize(request, env, url) {
  ...
  if (request.method === "POST") {
    ...
    if (!form) return htmlResponse("<h1>Invalid OAuth form</h1>", 400);
    params = new URLSearchParams(Array.from(form.entries()).map(([k, v]) => [k, String(v)]));
    readerToken = String(form.get("reader_token") || "");
  } else {
...
async function issueTokenPair(env, clientId, resource, scope) {
  ...
  await env.DB.batch([
    env.DB.prepare(`INSERT INTO mcp_oauth_tokens(token_hash,token_type,client_id,resource,scope,expires_at,revoked_at,parent_refresh_hash,created_at) VALUES(?,'access',?,?,?,?,NULL,?,?)`)
      .bind(accessHash, clientId, resource, scope, now + ACCESS_TTL_SECONDS, refreshHash, now),
