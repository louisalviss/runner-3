import app from "./artifact-library-reader-v13-media-id-sync-entry.js";
...
export default {
  async fetch(request, env, ctx) {
    ...
    const updated = original.includes(OLD_ID) ? original.split(OLD_ID).join(NEW_ID) : original;
    const headers = new Headers(response.headers);
    headers.delete("Content-Length");
    headers.set("X-Robots-Tag", ROBOTS);
    headers.set("X-R3-Reader-Runtime", "v14-wordboundary-cfi-media-id-all-sync");
