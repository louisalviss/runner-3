  ...
  <div class="wrap signal-stage__inner">
    ...
    <?php if ($featured): ?>
      <a class="signal-orbit" href="<?php echo esc_url(get_permalink($featured)); ?>" aria-label="00 FEATURE — Read <?php echo esc_attr(get_the_title($featured)); ?>">
        ...
        <span class="signal-orbit__image"><?php echo runner3_story_image_html($featured->ID, ['loading' => 'eager', 'fetchpriority' => 'high', 'sizes' => '(max-width: 767px) 80vw, 580px']); ?></span>
...
<section class="feature-scene" id="latest">
  <div class="wrap feature-scene__grid" data-reveal>
    ...
    <a class="scene-image" href="<?php echo esc_url(get_permalink($featured)); ?>" aria-label="Read <?php echo esc_attr(get_the_title($featured)); ?>"><?php echo runner3_story_image_html($featured->ID, ['loading' => 'lazy', 'sizes' => '(max-width: 900px) 92vw, 48vw']); ?></a>
  ...
  <div class="wrap reel-list">
    <?php foreach ($stories as $i => $post): $cat = get_the_category($post->ID)[0] ?? null; $n = $i + 1; ?>
      <article class="reel-story <?php echo $i % 2 ? 'reel-story--reverse' : ''; ?>" data-reveal>
        ...
        <a class="reel-image" href="<?php echo esc_url(get_permalink($post)); ?>" aria-label="Read <?php echo esc_attr(get_the_title($post)); ?>"><?php echo runner3_story_image_html($post->ID, ['loading' => 'lazy', 'sizes' => '(max-width: 800px) 92vw, 46vw']); ?></a>
        <div class="reel-copy">
