...
def click_outright(frame, page):
    ...
    while time.time() < deadline:
        ...
        try:
            info = frame.evaluate(
                """() => {
                  ...
                  const btn=e.closest('.feature-bar__market-button') || e.closest('[data-market="outright"]') || e.closest('[data-key="outright"]') || e.parentElement || e;
                  const text=(btn.innerText||btn.textContent||'').trim();
                  btn.click();
                  return {clicked:true,allDM,allDK,tag:e.tagName,cls:e.className||'',text};
                }"""
            )
            last_debug = info or last_debug
            clicked = bool((info or {}).get("clicked"))
            text = str((info or {}).get("text") or "")
            match = re.search(r"Outright\s*(\d+)", text, re.I | re.S)
            if match:
                badge_count = int(match.group(1))
        except Exception as exc:
            last_debug = {"selector_error": str(exc)}
            ...
            try:
                ...
                for i in range(min(loc.count(), 10)):
                    ...
                    try:
                        ...
                        clicked = True
                        last_debug = {"text_fallback": True, "text": text}
                        break
                    except Exception:
                        pass
            except Exception as exc:
        ...
        if "/outright" in fresh.url.lower():
            print("OUTRIGHT_NAV_OK", fresh.url, "badge=", badge_count, json.dumps(last_debug, ensure_ascii=False))
            return fresh, badge_count
        frame = fresh
...
def main():
    ...
    with sync_playwright() as p:
        ...
        # Critical boundary: normal Sports ShowAllOdds before this point must never
        # be merged into the Outright market universe.
        outright_response_mark = len(showall)
        frame, badge_count = click_outright(frame, page)
        ...
        outright_responses = showall[outright_response_mark:]
        if not outright_responses:
            # Defensive fallback: identify an Outright-shaped response by request metadata.
            for entry in showall:
                post = parse_post(entry.get("post") or "")
                game_id = str(post.get("GameId") or post.get("gameId") or "")
                if game_id == "999":
                    outright_responses.append(entry)
        if not outright_responses:
            raise RuntimeError(f"No Outright ShowAllOdds after route transition; all_showall={len(showall)}")

        league_names = {}
        team_names = {}
        response_debug = []
        for entry in outright_responses:
            ...
            response_debug.append({"post": parse_post(entry.get("post") or ""), "numeric_titles": numeric})
        ...
        print("OUTRIGHT_DISCOVERY", json.dumps({
            "all_showall": len(showall),
            "outright_showall": len(outright_responses),
            "badge_count": badge_count,
            "numeric_targets": len(targets),
            "responses": response_debug,
        }, ensure_ascii=False))
        if len(targets) < 20:
            raise RuntimeError(f"Unsafe Outright discovery count={len(targets)} responses={len(outright_responses)}")
        if badge_count and len(targets) > badge_count + 8:
            raise RuntimeError(f"Outright universe contamination suspected: badge={badge_count} targets={len(targets)}")
        ...
        result = {
            ...
            "coverage": {
                "showall_responses_total": len(showall),
                "showall_responses_outright": len(outright_responses),
                "badge_market_count": badge_count,
        ...
        # Write diagnostic payload before enforcing publish coverage so failed runs
        # remain inspectable; the workflow will not publish the release on failure.
        ...
        print("OUTRIGHT_RESULT", json.dumps(result["coverage"], ensure_ascii=False))
        print("OUTRIGHT_FAILURES", json.dumps(failures, ensure_ascii=False))
