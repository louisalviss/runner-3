...
def attach_event_week(reports: pd.DataFrame, w: pd.DataFrame) -> pd.DataFrame:
    ...
    for sym, r in reports.groupby('symbol', sort=False, observed=True):
        ...
        m = pd.merge_asof(rr, p, left_on='date_pub', right_on='week', direction='forward', allow_exact_matches=False)
...
def add_controls(ev: pd.DataFrame, all_reports: pd.DataFrame) -> pd.DataFrame:
    neutral = all_reports[all_reports['surprise_rank'].between(0.40, 0.60, inclusive='both') & all_reports['ret52_pre'].notna()].copy()
    ...
    for st, x0 in z.groupby('strategy', sort=False):
        for h in HORIZONS:
            ...
            if len(wm) >= 8:
                ...
                lo = hi = np.nan
            out.append({'slice': label, 'strategy': st, 'horizon': h, 'n': int(len(x)), 'matched_n': int(len(c)), 'signal_weeks': int(x['week'].nunique()), 'median_return': x[f'ret{h}'].median(), 'mean_return': x[f'ret{h}'].mean(), 'win_rate': (x[f'ret{h}'] > 0).mean(), 'median_excess': c[f'excess{h}'].median(), 'mean_excess': c[f'excess{h}'].mean(), 'beat_matched': (c[f'excess{h}'] > 0).mean(), 'ci_lo': lo, 'ci_hi': hi})
    return out
...
def main():
    ...
    v['pass'] = ((v['n'] >= 300) & (v['median_excess'] > 0.01) & (v['beat_matched'] >= 0.525) & (v['ci_lo'] > 0))
