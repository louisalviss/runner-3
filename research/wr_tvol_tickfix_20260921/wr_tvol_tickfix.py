...
def run_one(sym,bars):
    ...
    ref.calc_ind=cached
    try:
        rows,raw=canonical_run_case(base,ref,wr,inf,HISTORY_START,REPORT_START,REPORT_END,
                                    variant='canonical',anchor='start',use_session=True,eligible_signal=elig)
        sm['variants']['canonical_correct_tick']=raw
        for t in rows:
            d=datetime.fromtimestamp(int(t['signal'])/1000,tz=timezone.utc).astimezone(VN).date().isoformat()
            z={'symbol':sym,'audit_variant':'canonical_correct_tick','signal_date_vn':d,'t_labels':T_LABELS.get(d,[])}; z.update(t); alltr.append(z)
        for name,guarded in [('fixed_guarded_correct_tick',True),('fixed_pure_correct_tick',False)]:
...
def merge():
    ...
    report={'status':'COMPLETE' if not errors and len(list(OUT.glob('summary-*.json')))==SHARDS else 'PARTIAL','shards_seen':len(list(OUT.glob('summary-*.json'))),'errors':errors,
            'source':{'engine':'WR corrected Pine-compatible structural signal + corrected Binance tick metadata','market':'Binance USDT futures 5m','signal_period':'2025-01-01..2026-08-14','settlement_bars_through':'2026-08-31','volume_rule':'24h quote volume >=100M AND prior-completed-day avg10D dollar-volume proxy >200M','T_rule':'VN T-2,T-1,T0,T+2,T+3; T+1/white/weekend excluded','entry':'next-bar stop-entry','exit':'canonical_correct_tick keeps canonical lifecycle; fixed_* use +2.3R/-1R only','same_bar':'conservative SL','no_sweep':True},'variants':{}}
    for idx,name in enumerate(('canonical_correct_tick','fixed_guarded_correct_tick','fixed_pure_correct_tick')):
