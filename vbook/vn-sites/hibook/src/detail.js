load('config.js');
function execute(url){let r=fetch(url,{headers:HTTP_HEADERS}); if(!r.ok)return Response.error('HTTP '+r.status); let d=r.html();
 ...
 let author=d.select("[data-story-author]").first().text();
