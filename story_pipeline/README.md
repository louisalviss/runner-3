...
5. `merged/` — ghép các phần TiênVuc theo `chapter_map.json` explicit.
...
Không được ghép chỉ theo tiêu đề TiênVuc hoặc cứ N phần thành một chương. Một section của TiênVuc có thể bị chia thành nhiều phần web trong khi nguyên tác chứa nhiều chương khác nhau. `chapter_map.json` là boundary source-of-truth, audit được.
...
python story_pipeline/pipeline.py merge  --edited edited --output merged --map chapter_map.json
...
- Chỉ release batch khi `apply` và `qa` đều pass.
