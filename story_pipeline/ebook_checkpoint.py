...
Cloudflare Runner3 Core/D1 is the control plane only. Canonical ebook artifacts
live in the artifact store declared by each book manifest (Dropbox by default).
A chapter is marked complete only after its output artifact exists and its
content hash has been computed/verified by the caller.
"""
...
def inspect_resume(
    ...
    *,
    ...
    artifact_sha = file_sha256(artifact_path) if artifact_exists else None
    checkpoint_artifact_match = bool(
        checkpoint_identity_match and artifact_sha and position.get("artifact_sha256") == artifact_sha
    )
    if checkpoint_artifact_match:
        action = "skip"
        reason = "checkpoint-and-artifact-match"
    elif artifact_exists and not checkpoint_identity_match:
        action = "recover"
        reason = "artifact-exists-checkpoint-missing-or-stale"
    else:
        action = "edit"
        reason = "missing-or-mismatched-artifact"
    return {
        "ok": True,
        "book_id": manifest["book_id"],
        "chapter": chapter,
...
def save_chapter_complete(
    ...
    *,
    ...
    if not artifact.is_file() or artifact.stat().st_size <= 0:
        raise ValueError("artifact_file must exist and be non-empty before D1 success checkpoint")
    if not artifact_dropbox_path.strip():
        raise ValueError("artifact_dropbox_path is required; Dropbox is the canonical artifact store")
    if qa != "pass":
        raise ValueError("chapter cannot be completed unless qa='pass'")
    identity = build_identity(manifest, source_file, config_files, model=model)
...
def main() -> int:
    ...
    p = sub.add_parser("decision"); add_common(p); p.add_argument("--chapter", type=int, required=True); p.add_argument("--source-file", required=True); p.add_argument("--artifact-file"); p.add_argument("--config-file", action="append", default=[]); p.add_argument("--model")
    p = sub.add_parser("complete"); add_common(p); p.add_argument("--chapter", type=int, required=True); p.add_argument("--source-file", required=True); p.add_argument("--artifact-file", required=True); p.add_argument("--artifact-dropbox-path", required=True); p.add_argument("--config-file", action="append", default=[]); p.add_argument("--model"); p.add_argument("--recovered", action="store_true")
    ...
    if args.command == "sync-main":
        ...
        emit(inspect_resume(manifest, args.chapter, source_file=args.source_file, artifact_file=args.artifact_file, config_files=args.config_file, model=args.model, core_url=args.core_url))
    elif args.command == "complete":
        emit(save_chapter_complete(manifest, args.chapter, source_file=args.source_file, artifact_file=args.artifact_file, artifact_dropbox_path=args.artifact_dropbox_path, config_files=args.config_file, model=args.model, recovered=args.recovered, core_url=args.core_url))
